...
exports.EnhancedTagService = exports.TagMergeOperationSchema = exports.TagAnalyticsSchema = exports.TagRelationshipSchema = exports.TagSuggestionSchema = exports.TagSchema = void 0;
const events_1 = require("events");
const zod_1 = require("zod");
exports.TagSchema = zod_1.z.object({
    id: zod_1.z.string().uuid(),
    name: zod_1.z.string().min(1).max(50).regex(/^[a-zA-Z0-9-_\s]+$/, 'Invalid tag characters'),
    normalized_name: zod_1.z.string().min(1).max(50),
    slug: zod_1.z.string().regex(/^[a-z0-9-]+$/, 'Slug must contain only lowercase letters, numbers, and hyphens'),
...
class EnhancedTagService extends events_1.EventEmitter {
    ...
    async getSuggestions(query, options = {}) {
        const { context, includeAI = this.options.enableAISuggestions, maxResults = this.options.maxSuggestions } = options;
    ...
    async getAutocompleteSuggestions(input, excludeTags = [], limit = 10) {
        ...
        Array.from(this.tags.values()).forEach(tag => {
            if (excludeTags.includes(tag.id) || !tag.is_active)
                return;
            let score = 0;
    ...
    async getTagCloud(options = {}) {
        ...
        let tags = Array.from(this.tags.values()).filter(tag => tag.is_active &&
            (tag.usage_count || 0) >= minUsage);
        if (category) {
            tags = tags.filter(tag => tag.category_id === category);
        }
        if (timeRange) {
            tags = tags.filter(tag => tag.last_used &&
    ...
    async mergeTags(operation) {
        ...
        const sourceTags = source_tag_ids.map(id => {
            const tag = this.tags.get(id);
            if (!tag)
                throw new Error(`Source tag not found: ${id}`);
            return tag;
        });
        const targetTag = this.tags.get(target_tag_id);
        if (!targetTag)
            throw new Error(`Target tag not found: ${target_tag_id}`);
