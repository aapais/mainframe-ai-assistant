...
class SQLiteAdapter {
    ...
    async initialize() {
        try {
            ...
            throw new IStorageAdapter_1.ConnectionError(`Failed to initialize SQLite adapter: ${error.message}`, 'sqlite', error);
        }
    }
    async close() {
    ...
    async createEntry(entry) {
        ...
        `).run(id, entry.title, entry.problem, entry.solution, entry.category, entry.severity || 'medium', entry.created_by || 'system');
                ...
                this.errorCount++;
                throw new IStorageAdapter_1.QueryError(`Failed to create entry: ${error.message}`, 'sqlite', 'INSERT', error);
            }
        });
        return transaction();
    }
    async readEntry(id) {
      ...
      `).get(id);
            ...
            this.errorCount++;
            throw new IStorageAdapter_1.QueryError(`Failed to read entry: ${error.message}`, 'sqlite', 'SELECT', error);
        }
    }
    async updateEntry(id, updates) {
        ...
        const transaction = this.db.transaction(() => {
          ...
          `).run(...values);
                ...
                this.errorCount++;
                throw new IStorageAdapter_1.QueryError(`Failed to update entry: ${error.message}`, 'sqlite', 'UPDATE', error);
            }
        });
        return transaction();
    }
    async deleteEntry(id) {
        ...
        const transaction = this.db.transaction(() => {
            try {
                ...
                this.errorCount++;
                throw new IStorageAdapter_1.QueryError(`Failed to delete entry: ${error.message}`, 'sqlite', 'DELETE', error);
            }
        });
        return transaction();
    }
    async createEntries(entries) {
        ...
        `);
                ...
                for (const entry of entries) {
                    const id = (0, uuid_1.v4)();
                    entryStmt.run(id, entry.title, entry.problem, entry.solution, entry.category, entry.severity || 'medium', entry.created_by || 'system');
                    if (entry.tags && entry.tags.length > 0) {
                        entry.tags.forEach(tag => {
                            tagStmt.run(id, tag.toLowerCase().trim());
                        });
                ...
                this.errorCount++;
                throw new IStorageAdapter_1.QueryError(`Failed to create entries: ${error.message}`, 'sqlite', 'INSERT BATCH', error);
            }
        });
        return transaction();
    }
    async readEntries(ids) {
      ...
      `).all(...ids);
            ...
            this.errorCount++;
            throw new IStorageAdapter_1.QueryError(`Failed to read entries: ${error.message}`, 'sqlite', 'SELECT BATCH', error);
        }
    }
    async updateEntries(updates) {
        ...
        const transaction = this.db.transaction(() => {
            ...
            try {
                ...
                this.errorCount++;
                throw new IStorageAdapter_1.QueryError(`Failed to update entries: ${error.message}`, 'sqlite', 'UPDATE BATCH', error);
            }
        });
        return transaction();
    }
    async deleteEntries(ids) {
        ...
        const transaction = this.db.transaction(() => {
            ...
            try {
                ...
                this.errorCount++;
                throw new IStorageAdapter_1.QueryError(`Failed to delete entries: ${error.message}`, 'sqlite', 'DELETE BATCH', error);
            }
        });
        return transaction();
    }
    async searchEntries(query, options) {
        ...
        try {
            ...
            this.errorCount++;
            throw new IStorageAdapter_1.QueryError(`Failed to search entries: ${error.message}`, 'sqlite', 'SEARCH', error);
        }
    }
    async getPopularEntries(limit = 10) {
      ...
      `).all(limit);
            ...
            this.errorCount++;
            throw new IStorageAdapter_1.QueryError(`Failed to get popular entries: ${error.message}`, 'sqlite', 'SELECT', error);
        }
    }
    async getRecentEntries(limit = 10) {
      ...
      `).all(limit);
            ...
            this.errorCount++;
            throw new IStorageAdapter_1.QueryError(`Failed to get recent entries: ${error.message}`, 'sqlite', 'SELECT', error);
        }
    }
    async getSearchSuggestions(query, limit = 5) {
        ...
        LIMIT ?
      `).all(query, query, query, query, limit).map((row) => row.suggestion);
            this.queryCount++;
            return suggestions;
        }
        catch (error) {
            this.errorCount++;
            throw new IStorageAdapter_1.QueryError(`Failed to get search suggestions: ${error.message}`, 'sqlite', 'SELECT', error);
        }
    }
    async executeSQL(sql, params = []) {
        ...
        try {
            ...
            this.errorCount++;
            throw new IStorageAdapter_1.QueryError(`Failed to execute SQL: ${error.message}`, 'sqlite', sql, error);
        }
    }
    ...
    async import(data, format, options) {
        ...
        try {
            ...
            throw new IStorageAdapter_1.QueryError(`Failed to import data: ${error.message}`, 'sqlite', 'IMPORT', error);
        }
    }
    async getMetrics() {
        this.ensureInitialized();
        try {
            ...
            throw new IStorageAdapter_1.QueryError(`Failed to get metrics: ${error.message}`, 'sqlite', 'PRAGMA', error);
        }
    }
    async optimize() {
        this.ensureInitialized();
        ...
        try {
            ...
            throw new IStorageAdapter_1.QueryError(`Failed to optimize database: ${error.message}`, 'sqlite', 'OPTIMIZE', error);
        }
    }
    async healthCheck() {
        try {
    ...
    async getSchemaInfo() {
        ...
        try {
            ...
            throw new IStorageAdapter_1.QueryError(`Failed to get schema info: ${error.message}`, 'sqlite', 'SCHEMA', error);
        }
    }
    getConfig() {
        return { ...this.config };
    ...
    async initializeSchema() {
        const tables = this.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='kb_entries'").get();
        if (!tables) {
    ...
    updateFTSIndex(id, entry) {
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(id, entry.title, entry.problem, entry.solution, entry.tags?.join(' ') || '', entry.category);
        }
        catch (error) {
    ...
    updateFTSIndexForUpdate(id, updates) {
        try {
            ...
            if (updates.tags !== undefined) {
                ...
                const currentTags = this.db.prepare('SELECT GROUP_CONCAT(tag, " ") as tags FROM kb_tags WHERE entry_id = ?').get(id);
    ...
    executeExactSearch(query, options) {
      ...
      LIMIT ? OFFSET ?
    `).all(`%${query}%`, `%${query}%`, `%${query}%`, ...(options.category ? [options.category] : []), options.limit || 10, options.offset || 0);
        return entries.map(entry => ({
            entry: this.mapRowToEntry(entry),
            score: 100,
            matchType: 'exact',
    ...
    executeFTSSearch(query, options) {
      ...
      LIMIT ? OFFSET ?
    `).all(ftsQuery, ...(options.category ? [options.category] : []), options.limit || 10, options.offset || 0);
        return entries.map(entry => ({
            entry: this.mapRowToEntry(entry),
    ...
    executeFuzzySearch(query, options) {
      ...
      LIMIT ? OFFSET ?
    `).all(`%${query}%`, `%${query}%`, `%${query}%`, `%${query}%`, `%${query}%`, `%${query}%`, ...(options.category ? [options.category] : []), options.limit || 10, options.offset || 0);
        return entries.map(entry => ({
            entry: this.mapRowToEntry(entry),
    ...
    executeTagSearch(query, options) {
      ...
      ORDER BY e.usage_count DESC
      LIMIT ? OFFSET ?
    `).all(tag, ...(options.category ? [options.category] : []), options.limit || 10, options.offset || 0);
        return entries.map(entry => ({
            entry: this.mapRowToEntry(entry),
            score: 85,
            matchType: 'tag',
    ...
    async getIndexInfo() {
        const indexes = this.db.prepare("SELECT name, tbl_name, sql FROM sqlite_master WHERE type='index' AND name NOT LIKE 'sqlite_%'").all();
...
class SQLiteTransaction {
    ...
    async execute(sql, params = []) {
        ...
        try {
            ...
            throw new IStorageAdapter_1.QueryError(`Transaction execute failed: ${error.message}`, 'sqlite', sql, error);
        }
    }
    async commit() {
        if (!this.active) {
        ...
        try {
            ...
            throw new IStorageAdapter_1.TransactionError(`Transaction commit failed: ${error.message}`, 'sqlite', error);
        }
    }
    async rollback() {
        if (!this.active) {
        ...
        try {
            ...
            throw new IStorageAdapter_1.TransactionError(`Transaction rollback failed: ${error.message}`, 'sqlite', error);
        }
    }
    isActive() {
        return this.active;
