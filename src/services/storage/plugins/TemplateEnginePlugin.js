...
class TemplateEnginePlugin extends BaseStoragePlugin_1.BaseStoragePlugin {
    ...
    inferTemplateType(content) {
        if (content.includes('IDENTIFICATION DIVISION'))
            return 'cobol';
        if (content.includes('//JOB') || content.includes('//EXEC'))
            return 'jcl';
    ...
    async optimizeTemplate(templateId) {
        ...
        const generations = await this.adapter.executeSQL('SELECT * FROM template_generations WHERE template_id = ? ORDER BY created_at DESC LIMIT 100', [templateId]);
