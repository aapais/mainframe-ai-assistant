...
export class CodeAnalysisPlugin extends BaseStoragePlugin {
  ...
  async importCode(
    ...
    if (sizeMB > config.parsing.max_file_size_mb) {
      throw new Error(
        `File too large: ${sizeMB.toFixed(2)}MB (max: ${config.parsing.max_file_size_mb}MB)`
      );
    }
  ...
  private isParagraphStart(code: string): boolean {
    return (
      /^[A-Z0-9-]+\.?\s*$/.test(code) && !code.includes('DIVISION') && !code.includes('SECTION')
    );
  }
  ...
  private async calculateMetrics(codeFile: CodeFile): Promise<CodeMetrics> {
    ...
    const linesOfCode = lines.filter(
      line => line.trim() && !line.trim().startsWith('*') && line.length > 6
    ).length;
    ...
    if (codeFile.structure) {
      complexityScore =
        codeFile.structure.paragraphs.reduce((sum, p) => sum + p.complexity, 0) /
  ...
  private extractKeywords(text: string): string[] {
    return text
      ...
      .filter(
        word => !/^(the|and|for|with|that|this|from|have|been|will|would|could|should)$/.test(word)
      )
      .slice(0, 10);
  }
  ...
  private async loadExistingCodeFiles(): Promise<void> {
    const files = await this.adapter.executeSQL(
      'SELECT * FROM code_files ORDER BY created_at DESC'
    );
  ...
  private async loadExistingKBLinks(): Promise<void> {
    const links = await this.adapter.executeSQL(
      'SELECT * FROM kb_code_links ORDER BY created_at DESC'
    );
  ...
  async suggestImprovements(fileId: string): Promise<string[]> {
    ...
    if (codeFile.metrics) {
      if (codeFile.metrics.complexity_score > 10) {
        suggestions.push(
          'Consider breaking down complex paragraphs into smaller, more manageable sections'
        );
      }
