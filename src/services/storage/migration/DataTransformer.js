...
class DataTransformer extends events_1.EventEmitter {
    ...
    assessTransformationRisk(transformations) {
        ...
        for (const transformation of transformations) {
            if (!transformation.reversible)
                riskScore += 10;
            if (transformation.transformationType === 'merge')
                riskScore += 5;
            if (transformation.estimatedRows > 10000)
                riskScore += 3;
            if (transformation.sql.includes('DELETE'))
                riskScore += 8;
        }
        if (riskScore >= 20)
            return 'critical';
    ...
    getTransformationsForMigration(migration) {
        return Array.from(this.transformationRegistry.values()).filter(t => migration.up.includes(t.targetTable) || migration.up.includes(t.sourceTable));
    }
