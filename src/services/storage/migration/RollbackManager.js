...
class RollbackManager extends events_1.EventEmitter {
    ...
    assessDataRecoveryNeed(steps) {
        return steps.some(step => step.dataImpact === 'significant' ||
    ...
    async createPreRollbackBackup(planId) {
        ...
        const backupPath = path_1.default.join(path_1.default.dirname(this.db.name), `backup_pre_rollback_${planId}_${timestamp}.db`);
    ...
    `).run('', step.step, step.migrationVersion, 'rollback', result.success ? 1 : 0, result.duration, result.error || null);
    }
    sleep(ms) {
    ...
    async validateRollbackStep(step) { return true; }
    ...
    async loadRollbackCheckpoint(checkpointId) { return null; }
    async validateRollbackDependencies() { }
    async enableRollbackSafetyMeasures() { }
    async versionExists(version) { return true; }
    async checkDataDependencies(version) { return []; }
    async identifyBreakingChanges(version) { return []; }
