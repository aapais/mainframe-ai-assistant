...
class StorageService extends events_1.EventEmitter {
    ...
    async initialize(config) {
        try {
            ...
            this.config = config;
            this.adapter = StorageFactory_1.StorageFactory.createAdapter(config.database.type, config.database);
            await this.adapter.initialize();
            this.backupService = new BackupService_1.BackupService(this.adapter, config.backup);
            this.migrationService = new MigrationService_1.MigrationService(this.adapter, config.mvp.version);
            this.performanceMonitor = new PerformanceMonitor_1.PerformanceMonitor(this.adapter, config.performance.monitoring);
    ...
    async readEntries(ids) {
        ...
        if (missedIds.length > 0) {
            ...
            await Promise.all(fetchedEntries.map(entry => entry ? this.cacheManager.set(`entry:${entry.id}`, entry, 300000) : Promise.resolve()));
            let fetchIndex = 0;
