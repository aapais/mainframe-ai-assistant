...
class RestoreService extends events_1.EventEmitter {
    ...
    async findNearestRestorePoint(targetTime) {
        ...
        return validPoints.sort((a, b) => Math.abs(targetTime.getTime() - a.timestamp.getTime()) -
            Math.abs(targetTime.getTime() - b.timestamp.getTime()))[0];
    }
...
function calculateRestoreTime(backupChain) {
    ...
    if (backupChain.incrementalBackups.length > 0) {
        const incrementalTime = backupChain.incrementalBackups.reduce((sum, backup) => sum + Math.max(backup.size / (1024 * 1024 * 2), 5), 0);
...
function getRestoreComplexity(backupChain) {
    ...
    const hasLargeDifferential = backupChain.differentialBackup &&
        backupChain.differentialBackup.size > 100 * 1024 * 1024;
