...
export declare abstract class AbstractBackupStrategy {
    ...
    abstract execute(adapter: IStorageAdapter, progress: BackupProgress, context?: BackupExecutionContext): Promise<Buffer>;
    abstract getRequiredDependencies(): string[];
    abstract validatePreconditions(adapter: IStorageAdapter, context?: BackupExecutionContext): Promise<{
        valid: boolean;
        issues: string[];
    }>;
    ...
    protected updateProgress(progress: BackupProgress, phase: string, percentage: number, bytesProcessed?: number): void;
...
export declare class FullBackupStrategy extends AbstractBackupStrategy {
    constructor();
    execute(adapter: IStorageAdapter, progress: BackupProgress, context?: BackupExecutionContext): Promise<Buffer>;
    getRequiredDependencies(): string[];
    validatePreconditions(adapter: IStorageAdapter, context?: BackupExecutionContext): Promise<{
        valid: boolean;
        issues: string[];
    }>;
...
export declare function getRecommendedStrategy(dataSize: number, changeFrequency: 'low' | 'medium' | 'high', storageConstraints: 'none' | 'limited' | 'critical'): BackupStrategyType;
