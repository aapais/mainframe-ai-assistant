...
class BackupService extends events_1.EventEmitter {
    ...
    async listBackups(filter) {
        ...
        if (filter?.destination) {
            filteredJobs = filteredJobs.filter(job => job.destinations.some(dest => dest.id === filter.destination));
        }
        if (filter?.fromDate) {
        ...
        if (filter?.tags && filter.tags.length > 0) {
            filteredJobs = filteredJobs.filter(job => job.request.tags && job.request.tags.some(tag => filter.tags.includes(tag)));
        }
    ...
    async initializeStrategies() {
        const { BackupStrategy } = await Promise.resolve().then(() => tslib_1.__importStar(require('./BackupStrategy')));
    ...
    async createBackupJob(jobId, request) {
        ...
        if (request.destinations && request.destinations.length > 0) {
            destinations = this.config.destinations.filter(d => request.destinations.includes(d.id) && d.enabled);
        }
        else {
    ...
    async executeBackupJob(job) {
        try {
            ...
            this.emit('backup:progress', job);
            const uploadPromises = job.destinations.map(dest => this.uploadToDestination(dest, backupData, job));
    ...
    async validateBackupResults(job) {
        for (const result of job.results) {
            if (result.success && !result.validationResult) {
                result.validationResult = await this.validator.validate(result.backupPath, result.checksums.original);
            }
        }
    }
    ...
    async storeJobMetadata(job) {
        await this.adapter.executeSQL(`INSERT INTO backup_jobs (
    ...
    hydrateJobFromRow(row) {
        return {
            ...
            destinations: this.config.destinations.filter(d => JSON.parse(row.request_data).destinations?.includes(d.id) ||
    ...
    async getDestinationMetrics() {
        return Promise.all(this.config.destinations.map(async (dest) => {
