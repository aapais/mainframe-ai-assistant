...
class ValidationService {
    ...
    async validateBatch(batch, batchIndex = 0, options = {}) {
        try {
            ...
            const adjustedWarnings = batchValidation.warnings.map(warning => `Batch ${batchIndex}: ${warning}`);
            return {
                ...batchValidation,
        ...
        this.businessRules.set('content-consistency', [
            {
                ...
                validator: (value, record) => {
                    ...
                    const commonWords = problemWords.filter(word => word.length > 3 && solutionWords.includes(word));
    ...
    validateTags(value) {
        ...
        if (Array.isArray(value)) {
            const invalidTags = value.filter(tag => typeof tag !== 'string' || tag.length === 0 || tag.length > 20);
    ...
    findClosestCategory(input, validCategories) {
        ...
        const directMatches = validCategories.filter(cat => cat.toLowerCase().includes(inputLower) || inputLower.includes(cat.toLowerCase()));
