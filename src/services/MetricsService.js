...
class MetricsService extends events_1.EventEmitter {
  ...
  async recordSearch(query, results) {
    try {
      ...
      throw new services_1.DatabaseError('Failed to record search metrics', 'recordSearch', {
        query,
        error,
      });
    }
  ...
  async recordUsage(entryId, action, userId, metadata) {
    try {
      ...
      throw new services_1.DatabaseError('Failed to record usage metrics', 'recordUsage', {
        entryId,
        action,
        error,
      });
