...
class ModelValidationService {
    ...
    async validateModel(modelConfig) {
        ...
        try {
            ...
            if (response && response.data && response.data[0] && response.data[0].embedding) {
                ...
                console.log(`✅ Model ${testModel} validated successfully (${result.embeddingDimensions} dimensions)`);
                return result;
            }
    ...
    async findWorkingModel(modelConfigs) {
        ...
        for (let i = 0; i < modelConfigs.length; i++) {
            ...
            console.log(`Testing model ${i + 1}/${modelConfigs.length}: ${config.model} (${config.provider})`);
