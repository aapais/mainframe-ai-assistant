  ...
  async export(data, config, filePath) {
    try {
      const jsonContent = config.options.prettyPrint
        ...
        : JSON.stringify(data);
      await fs_1.promises.writeFile(filePath, jsonContent, {
        encoding: config.options.encoding || 'utf8',
      });
      const stats = await fs_1.promises.stat(filePath);
      return {
        success: true,
...
class XMLFormatHandler {
  ...
  async export(data, config, filePath) {
    try {
      ...
      xmlContent += '</root>';
      await fs_1.promises.writeFile(filePath, xmlContent, {
        encoding: config.options.encoding || 'utf8',
      });
      const stats = await fs_1.promises.stat(filePath);
      return {
...
class HTMLFormatHandler {
  ...
  async export(data, config, filePath) {
    try {
      ...
      htmlContent += '\n</table>\n</body>\n</html>';
      await fs_1.promises.writeFile(filePath, htmlContent, {
        encoding: config.options.encoding || 'utf8',
      });
      const stats = await fs_1.promises.stat(filePath);
      return {
