...
class AlertManager extends events_1.EventEmitter {
    ...
    createAlertRule(name, dataSource, condition, thresholds, notifications, schedule, createdBy, description, suppressions) {
        const rule = {
            id: this.generateId(),
            name,
            description,
    ...
    getMetrics() {
        ...
        const successfulNotifications = recentAlerts.reduce((sum, a) => sum + a.notifications.filter(n => n.status === 'sent').length, 0);
