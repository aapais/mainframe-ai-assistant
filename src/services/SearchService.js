...
class SearchService {
    ...
    queryOptimizationCache = new Map();
    ...
    async search(query, entries, options = {}) {
        ...
        try {
            ...
            if (options.useAI !== false && this.geminiConfig && entries.length <= 100) {
                searchPromises.push(this.performAISearch(normalizedQuery, entries, options)
                    .catch(error => {
            ...
            throw new services_1.SearchError(`Enhanced search failed: ${error.message}`, query, { originalError: error, options, processingTime: performance.now() - startTime });
        }
    }
    async searchWithAI(query, entries, options = {}) {
        if (!this.geminiConfig) {
            throw new services_1.ServiceError('AI search not configured', 'AI_NOT_CONFIGURED');
        }
    ...
    async explain(query, result) {
        ...
        switch (result.matchType) {
            ...
            case 'category':
                ...
                break;
            case 'tag':
                const matchingTags = result.entry.tags.filter(tag => query.toLowerCase().includes(tag.toLowerCase()) ||
    ...
    generateInstantCacheKey(query, options) {
    ...
    setInInstantCache(key, results, ttl) {
    ...
    async performOptimizedFTSSearch(query, entries, options) {
        if (!this.database) {
    ...
    intelligentMergeResults(resultSets, options) {
        ...
        resultSets.forEach(results => {
            results.forEach(result => {
                ...
                if (existing) {
                    const sourceWeight = sourceWeights[result.metadata?.source] || 0.5;
    ...
    streamOptimizedResults(results, limit) {
    ...
    detectQueryIntent(query) {
        ...
        if (errorPatterns.test(query))
            return 'error_resolution';
        if (howToPatterns.test(query))
            return 'how_to';
        if (troublePatterns.test(query))
            return 'troubleshooting';
        return 'general_info';
    }
    calculateRecentSuccessBoost(entry) {
        ...
        if (recencyDays <= 7 && successRate > 0.8)
            return 0.2;
        if (recencyDays <= 30 && successRate > 0.7)
            return 0.1;
        return 0;
    }
    calculateContextBoost(entry, userContext) {
        if (entry.category.toLowerCase().includes(userContext.toLowerCase())) {
            return 0.15;
        }
        return 0;
    }
    calculateSemanticBoost(entry, queryWords) {
    ...
    generateContextualHighlights(entry, queryWords) {
        const highlights = [];
    ...
    async performLocalSearch(query, entries, options) {
        const results = [];
        const queryWords = this.tokenizeQuery(query);
        entries.forEach(entry => {
    ...
    async performAISearch(query, entries, options) {
        if (!this.geminiConfig) {
            throw new services_1.AIServiceError('Gemini API not configured', 'gemini');
        }
        try {
            const prompt = this.buildAISearchPrompt(query, entries, options);
            const response = await axios_1.default.post(`https://generativelanguage.googleapis.com/v1beta/models/${this.geminiConfig.model}:generateContent?key=${this.geminiConfig.apiKey}`, {
                contents: [{
                        parts: [{ text: prompt }]
                    }],
                generationConfig: {
            ...
            if (axios_1.default.isAxiosError(error)) {
                ...
                throw new services_1.AIServiceError(`Gemini API error: ${axiosError.message}`, 'gemini', { status: axiosError.response?.status, data: axiosError.response?.data });
            }
            throw new services_1.AIServiceError(`AI search failed: ${error.message}`, 'gemini', { originalError: error });
        }
    }
    buildAISearchPrompt(query, entries, options) {
        const relevantEntries = entries.slice(0, 50);
        const entriesList = relevantEntries.map((entry, index) => `${index}: [${entry.category}] ${entry.title}\nProblem: ${entry.problem.substring(0, 200)}...\nSolution: ${entry.solution.substring(0, 200)}...\nTags: ${entry.tags.join(', ')}\n`).join('\n');
...
Return only the JSON array, nothing else.`;
    ...
    parseAIResponse(response, entries, query) {
        try {
    ...
    calculateLocalScore(entry, query, queryWords, options) {
        let score = 0;
        ...
        if (options.tags) {
            const matchingTags = options.tags.filter(tag => entry.tags.some(entryTag => entryTag.toLowerCase().includes(tag.toLowerCase())));
    ...
    determineMatchType(entry, query, queryWords) {
        const queryLower = query.toLowerCase();
        ...
        const hasTagMatch = entry.tags.some(tag => queryWords.some(word => tag.toLowerCase().includes(word)));
        if (hasTagMatch) {
            return 'tag';
        }
    ...
    mergeSearchResults(localResults, aiResults, options) {
    ...
    filterAndSortResults(results, options) {
        ...
        if (options.tags && options.tags.length > 0) {
            filtered = filtered.filter(result => options.tags.some(tag => result.entry.tags.some(entryTag => entryTag.toLowerCase().includes(tag.toLowerCase()))));
        }
    ...
    findHighlightsInText(text, queryWords, field, highlights) {
    ...
    async recordSearch(query, results, options) {
        const searchQuery = {
            text: query,
            options,
            timestamp: new Date(),
