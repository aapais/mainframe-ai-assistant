  ...
  async performAISearch(query, entries, options) {
    ...
    try {
      ...
      if (axios_1.default.isAxiosError(error)) {
        ...
        throw new services_1.AIServiceError(`Gemini API error: ${axiosError.message}`, 'gemini', {
          status: axiosError.response?.status,
          data: axiosError.response?.data,
        });
      }
      throw new services_1.AIServiceError(`AI search failed: ${error.message}`, 'gemini', {
        originalError: error,
      });
    }
  }
