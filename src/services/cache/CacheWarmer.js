...
class CacheWarmer {
    ...
    async warmCache(strategy, userContext) {
        if (this.isWarming) {
            throw new services_1.ServiceError('Cache warming already in progress', 'WARMING_IN_PROGRESS', 429);
        }
        this.isWarming = true;
        ...
        try {
            ...
            switch (strategy) {
                ...
                case 'machine-learning':
                    ...
                    break;
                default:
                    throw new services_1.ServiceError(`Unknown warming strategy: ${strategy}`, 'INVALID_STRATEGY', 400);
            }
    ...
    async warmForUser(userContext) {
        ...
        try {
            ...
            throw new services_1.ServiceError(`User-specific warming failed: ${error.message}`, 'USER_WARMING_ERROR', 500, { userId: userContext.userId, originalError: error });
        }
    }
    async predictiveWarming(timeWindow = 3600000) {
        ...
        try {
            ...
            throw new services_1.ServiceError(`Predictive warming failed: ${error.message}`, 'PREDICTIVE_WARMING_ERROR', 500, { originalError: error });
        }
    }
    getWarmingStats() {
    ...
    scheduleWarming(strategyName, schedule) {
        const interval = this.calculateNextRun(this.config.strategies.find(s => s.name === strategyName)?.priority || 'normal');
        setTimeout(async () => {
            try {
    ...
    selectOptimalStrategies(performanceMetrics) {
        return this.config.strategies
            ...
            if (performanceMetrics.hitRate < 0.7) {
                if (a.name === 'popular-queries')
                    aScore += 0.3;
                if (b.name === 'popular-queries')
                    bScore += 0.3;
            }
            if (performanceMetrics.avgResponseTime > 1000) {
                if (a.name === 'predictive-user')
                    aScore += 0.2;
                if (b.name === 'predictive-user')
                    bScore += 0.2;
            }
