...
class CacheServiceIntegration extends events_1.EventEmitter {
    ...
    async getHealthStatus() {
        ...
        const hasHealthyCache = (status.legacy?.status === 'healthy') ||
            (status.advanced?.status === 'healthy');
        if (!hasHealthyCache) {
            status.overall = 'unhealthy';
        }
        else if ((status.legacy?.status === 'degraded') ||
            ...
            for (let i = 0; i < backup.entries.length; i += batchSize) {
                ...
                await Promise.allSettled(batch.map(async (entry) => {
                    try {
                        ...
                        this.migrationStatus.errors.push(`Failed to migrate key ${entry.key}: ${error.message}`);
                    }
                }));
    ...
    setupPerformanceMonitoring() {
        if (!this.orchestrator)
            return;
        this.performanceMonitor = new index_1.PerformanceMonitor(this.orchestrator, this.cdnIntegration);
