...
class PredictiveCache extends events_1.EventEmitter {
    ...
    updateQueryPatterns(pattern, event) {
        const existingPattern = pattern.queryPatterns.find(p => this.calculateSimilarity(p.pattern, event.query) > 0.8);
        if (existingPattern) {
    ...
    calculatePredictionScore(prediction) {
        ...
        return (prediction.confidence * confidenceWeight +
    ...
    updateStats() {
        ...
        const recentPredictions = this.predictionHistory
            .filter(p => Date.now() - p.timestamp < 60 * 60 * 1000);
