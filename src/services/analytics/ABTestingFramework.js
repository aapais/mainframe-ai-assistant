...
class ABTestingFramework extends events_1.EventEmitter {
    ...
    async analyzeTestResults(testId) {
        ...
        const variantResults = await Promise.all(test.variants.map(async (variant) => {
            ...
            metrics[test.primaryMetric] = {
                ...
                confidenceInterval: this.calculateConfidenceInterval(primaryMetricValue.value, primaryMetricValue.standardError, test.successCriteria.confidenceLevel),
            ...
            for (const metricName of test.secondaryMetrics) {
                ...
                metrics[metricName] = {
                    ...
                    confidenceInterval: this.calculateConfidenceInterval(metricValue.value, metricValue.standardError, test.successCriteria.confidenceLevel),
                    sampleSize: participants
                };
            }
            return {
        ...
        const controlVariant = variantResults.find(v => test.variants.find(tv => tv.id === v.variantId)?.isControl);
        if (controlVariant) {
            variantResults.forEach(variant => {
                if (variant.variantId === controlVariant.variantId)
                    return;
                const significance = this.calculateStatisticalSignificance(controlVariant.metrics[test.primaryMetric], variant.metrics[test.primaryMetric]);
                variant.significance[test.primaryMetric] = significance;
                test.secondaryMetrics.forEach(metricName => {
                    const metricSignificance = this.calculateStatisticalSignificance(controlVariant.metrics[metricName], variant.metrics[metricName]);
    ...
    async performBayesianAnalysis(testId) {
        ...
        const allSamples = variants.map(v => this.sampleBetaDistribution(v.posteriorDistribution.alpha, v.posteriorDistribution.beta, 10000));
        variants.forEach((variant, index) => {
            let wins = 0;
            for (let i = 0; i < 10000; i++) {
                const variantSample = allSamples[index][i];
                const isWinner = allSamples.every((samples, idx) => idx === index || variantSample > samples[i]);
                if (isWinner)
                    wins++;
            }
        ...
        const bestVariant = variants.reduce((best, current) => current.probabilityOfBeingBest > best.probabilityOfBeingBest ? current : best);
        const recommendation = {
            recommendedAction: bestVariant.probabilityOfBeingBest > 0.95 ? 'promote' :
                bestVariant.probabilityOfBeingBest > 0.8 ? 'continue' : 'stop',
    ...
    async getDashboardData() {
        ...
        const activeTestsData = await Promise.all(activeTests.map(async (test) => {
        ...
        const completedTests = allTests
            ...
            .slice(0, 5);
        const recentResults = await Promise.all(completedTests.map(async (test) => {
            ...
            const controlResults = results.variants.find(v => test.variants.find(tv => tv.id === v.variantId)?.isControl);
    ...
    calculateStatisticalSignificance(control, variant) {
        const pooledStandardError = Math.sqrt(Math.pow(control.standardError, 2) + Math.pow(variant.standardError, 2));
    ...
    generateRecommendations(test, variants) {
        ...
        if (winningVariant) {
            ...
            nextSteps.push('Implement winning variant', 'Monitor post-launch metrics', 'Plan follow-up tests based on learnings');
        }
        else {
            reasoning = 'No variant achieved statistical significance';
            nextSteps.push('Continue test to reach significance', 'Review test design and metrics', 'Consider increasing traffic allocation');
        }
        return {
            winningVariant,
            confidence,
    ...
    async generateTestAlerts(activeTests) {
        ...
        for (const test of activeTests) {
            ...
            if (testDuration > 14 * 24 * 60 * 60 * 1000 && !results.statisticalAnalysis.hasReachedSignificance) {
                alerts.push({
