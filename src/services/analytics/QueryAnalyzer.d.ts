...
export type QueryPatternType = 'keyword_search' | 'phrase_search' | 'boolean_complex' | 'field_specific' | 'fuzzy_search' | 'wildcard_search' | 'range_search' | 'nested_search' | 'technical_lookup' | 'troubleshooting' | 'procedural_search';
...
export declare class QueryAnalyzer {
    ...
    analyzeQuery(query: string, parsedQuery: ParsedQuery, executionTime: number, resultCount: number, userId?: string): {
        pattern: QueryPattern;
    ...
    findSimilarQueries(query: string, limit?: number): Array<{
        query: string;
        similarity: number;
        pattern: QueryPattern;
    }>;
