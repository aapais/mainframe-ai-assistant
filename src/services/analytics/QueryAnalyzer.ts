...
export class QueryAnalyzer {
   ...
   */
  public findSimilarQueries(query: string, limit: number = 10): Array<{
    query: string;
    similarity: number;
    pattern: QueryPattern;
  }> {
  ...
  public optimizeQuery(query: string): {
    ...
    const successfulSimilar = similarQueries.filter(
      sq => sq.pattern.successRate > 0.8
    );
  ...
  private calculateQueryComplexity(parsedQuery: ParsedQuery): QueryComplexity {
    ...
    score += parsedQuery.terms.filter(t => 
      t.operator === 'AND' || t.operator === 'OR'
    ).length * 2;
  ...
  private recalculateClusterMetrics(cluster: QueryCluster): void {
    ...
    for (let i = 0; i < cluster.queries.length; i++) {
      for (let j = i + 1; j < cluster.queries.length; j++) {
        totalSimilarity += this.calculateQuerySimilarity(
          cluster.queries[i], 
          cluster.queries[j]
        );
        comparisons++;
  ...
  private calculatePatternDistribution(history: typeof this.queryHistory): Record<QueryPatternType, number> {
  ...
  private calculateComplexityDistribution(history: typeof this.queryHistory): Record<QueryComplexity, number> {
  ...
  private calculateIntentDistribution(history: typeof this.queryHistory): Record<QueryIntent, number> {
