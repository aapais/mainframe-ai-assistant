...
class RelevanceScorer extends events_1.EventEmitter {
    ...
    generateExplanation(signals, overallScore) {
        const topSignals = signals
            ...
            .slice(0, 3);
        const explanations = topSignals.map(signal => `${signal.source}: ${(signal.score * 100).toFixed(1)}% (${signal.type})`);
    ...
    analyzeQueryRelation(query, previousQueries) {
        const hasRelatedQuery = previousQueries.some(prev => this.calculateQuerySimilarity(query, prev) > 0.5);
    ...
    classifyQueryIntent(query) {
        if (query.toLowerCase().includes('how to') || query.includes('?'))
    ...
    classifyContentType(resultContent) {
        if (resultContent.metadata?.type)
            return resultContent.metadata.type;
        if (resultContent.title.toLowerCase().includes('tutorial'))
            return 'tutorial';
        if (resultContent.title.toLowerCase().includes('review'))
            return 'review';
        return 'article';
    }
