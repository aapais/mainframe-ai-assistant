...
class SearchIntentClassifier {
    ...
    classifyIntent(query, parsedQuery, context) {
        ...
        const isMultiIntent = alternatives.length > 0 &&
            alternatives[0].confidence > this.config.multiIntentThreshold;
    ...
    getIntentDistribution(timeRange) {
        ...
        const distribution = {};
        const allIntents = [
            'informational', 'navigational', 'transactional', 'investigational',
            'troubleshooting', 'procedural', 'comparative', 'definitional',
        ...
        for (const intent of allIntents) {
            ...
            distribution[intent] = {
                ...
                avgConfidence: intentData.length > 0 ?
                    intentData.reduce((sum, d) => sum, 0) / intentData.length : 0,
                successRate: intentData.length > 0 ?
                    successfulInteractions.length / intentData.length : 0
            };
        }
        return distribution;
    }
    ...
    initializePatterns() {
        this.addIntentPattern('informational', [
            { type: 'keyword', pattern: /\b(what|how|why|when|where|explain|define|describe)\b/i, weight: 0.8, examples: ['What is COBOL', 'How does JCL work'] },
        ...
        this.addIntentPattern('navigational', [
            { type: 'keyword', pattern: /\b(find|locate|search|show|display|list)\b/i, weight: 0.8, examples: ['Find COBOL examples', 'Show JCL procedures'] },
            { type: 'keyword', pattern: /\b(documentation|manual|guide|reference)\b/i, weight: 0.7, examples: ['COBOL documentation'] },
            { type: 'phrase', pattern: 'where is', weight: 0.6, examples: ['Where is the DB2 manual'] }
        ]);
        this.addIntentPattern('transactional', [
            { type: 'keyword', pattern: /\b(download|export|save|generate|create|build|compile)\b/i, weight: 0.9, examples: ['Download JCL template', 'Generate COBOL program'] },
            { type: 'keyword', pattern: /\b(install|setup|configure|implement)\b/i, weight: 0.8, examples: ['Install CICS'] }
        ]);
        this.addIntentPattern('troubleshooting', [
            { type: 'keyword', pattern: /\b(error|problem|issue|fix|solve|debug|troubleshoot)\b/i, weight: 0.9, examples: ['Fix COBOL error', 'Debug JCL problem'] },
            { type: 'keyword', pattern: /\b(not working|failed|exception|abend)\b/i, weight: 0.8, examples: ['JCL not working'] },
            { type: 'phrase', pattern: 'how to fix', weight: 0.9, examples: ['How to fix SQLCODE error'] }
        ]);
        this.addIntentPattern('procedural', [
            { type: 'keyword', pattern: /\b(step|steps|procedure|process|tutorial|guide)\b/i, weight: 0.8, examples: ['Steps to compile COBOL'] },
            { type: 'phrase', pattern: /how to\b/i, weight: 0.7, examples: ['How to create dataset'] },
            { type: 'keyword', pattern: /\b(walkthrough|instructions|methodology)\b/i, weight: 0.7, examples: ['CICS installation walkthrough'] }
        ]);
        this.initializeRemainingPatterns();
    }
    initializeRemainingPatterns() {
        this.addIntentPattern('investigational', [
            { type: 'keyword', pattern: /\b(analyze|research|investigate|study|compare|evaluate)\b/i, weight: 0.8, examples: ['Analyze COBOL performance'] },
            { type: 'keyword', pattern: /\b(best practices|comparison|alternatives|options)\b/i, weight: 0.7, examples: ['COBOL best practices'] }
        ]);
        this.addIntentPattern('comparative', [
            { type: 'keyword', pattern: /\b(vs|versus|compared to|difference|compare)\b/i, weight: 0.9, examples: ['COBOL vs PL/I'] },
            { type: 'keyword', pattern: /\b(which|better|pros and cons|advantages)\b/i, weight: 0.7, examples: ['Which is better CICS or IMS'] }
        ]);
        this.addIntentPattern('definitional', [
            { type: 'keyword', pattern: /\b(define|definition|meaning|what is|what are)\b/i, weight: 0.9, examples: ['Define VSAM'] },
            { type: 'keyword', pattern: /\b(acronym|stands for|abbreviation)\b/i, weight: 0.8, examples: ['What does JCL stand for'] }
        ]);
        this.addIntentPattern('exploratory', [
            { type: 'keyword', pattern: /\b(explore|overview|introduction|learn about)\b/i, weight: 0.8, examples: ['Explore mainframe concepts'] },
            { type: 'keyword', pattern: /\b(topics|subjects|areas|domains)\b/i, weight: 0.6, examples: ['Mainframe topics'] }
        ]);
        this.addIntentPattern('verification', [
            { type: 'keyword', pattern: /\b(verify|validate|check|confirm|ensure)\b/i, weight: 0.8, examples: ['Verify JCL syntax'] },
            { type: 'keyword', pattern: /\b(correct|right|accurate|valid)\b/i, weight: 0.7, examples: ['Is this COBOL correct'] }
        ]);
    }
    ...
    initializeDomainVocabulary() {
        const vocabulary = {
            'mainframe': 1.0, 'z/os': 1.0, 'mvs': 0.9, 'zvm': 0.9, 'vse': 0.8,
            'cics': 1.0, 'ims': 0.9, 'db2': 1.0, 'idms': 0.8, 'adabas': 0.8,
            'cobol': 1.0, 'pli': 0.8, 'assembler': 0.9, 'jcl': 1.0, 'rexx': 0.8,
            'rpg': 0.7, 'fortran': 0.6, 'sql': 0.9,
            'vsam': 1.0, 'qsam': 0.8, 'isam': 0.7, 'dataset': 0.9, 'gdg': 0.8,
            'catalog': 0.8, 'volume': 0.7, 'dasd': 0.8, 'tape': 0.7,
            'jobcard': 0.9, 'proclib': 0.8, 'steplib': 0.8, 'sysin': 0.7,
            'sysout': 0.7, 'allocation': 0.8, 'disposition': 0.7,
            'ispf': 0.9, 'sdsf': 0.8, 'fileaid': 0.7, 'endevor': 0.8,
            'changeman': 0.7, 'abendaid': 0.8, 'xpediter': 0.7,
            'rmf': 0.8, 'smf': 0.8, 'vtam': 0.8, 'racf': 0.9, 'acf2': 0.8,
            'topsecret': 0.8, 'omegamon': 0.7
        };
                ...
                for (const patternItem of pattern.patterns) {
                    ...
                    if (patternItem.type === 'keyword' || patternItem.type === 'phrase') {
                        if (patternItem.pattern instanceof RegExp) {
                            ...
                            patternMatch = query.toLowerCase().includes(patternItem.pattern.toLowerCase()) ? patternItem.weight : 0;
                        }
                    }
    ...
    calculateDomainSpecificity(features) {
        const domainTerms = features.mainframeConcepts.length +
    ...
    extractVerbs(query) {
        const commonVerbs = [
            'find', 'search', 'get', 'show', 'display', 'create', 'build', 'setup',
            'configure', 'install', 'fix', 'solve', 'debug', 'troubleshoot',
    ...
    extractModifiers(query) {
        const modifiers = ['best', 'better', 'latest', 'new', 'old', 'fast', 'slow', 'easy', 'difficult'];
    ...
    extractUrgencyIndicators(query) {
        const urgencyTerms = [
            ...
            'down', 'failed', 'not working', 'stopped', 'crash', 'hang'
        ];
    ...
    calculateSpecificityLevel(query, parsedQuery) {
        ...
        if (query.includes('"'))
            score += 0.3;
        if (parsedQuery.terms.some(t => t.field))
            score += 0.3;
    ...
    calculateAbstractionLevel(query) {
        const abstractTerms = [
            'concept', 'theory', 'principle', 'overview', 'introduction',
            'general', 'basic', 'fundamental', 'architecture', 'design'
        ];
        const abstractMatches = abstractTerms.filter(term => new RegExp(`\\b${term}\\b`, 'i').test(query)).length;
    ...
    extractBusinessTerms(query) {
        const businessTerms = [
            'cost', 'budget', 'performance', 'efficiency', 'productivity',
            'compliance', 'audit', 'security', 'governance', 'process'
        ];
    ...
    updateUserProfile(userId, intent, satisfied) {
        ...
        const totalQueries = Array.from(profile.intentFrequency.values())
    ...
    filterLearningData(timeRange) {
        ...
        return this.learningData.filter(data => data.timestamp >= timeRange.from && data.timestamp <= timeRange.to);
    }
    calculateModelPerformance() {
        ...
        const correctClassifications = dataWithFeedback.filter(d => d.classifiedIntent === d.actualIntent).length;
