...
export class SearchIntentClassifier {
   ...
   */
  public getIntentDistribution(timeRange?: { from: number; to: number }): Record<SearchIntent, {
    count: number;
    percentage: number;
    ...
    const distribution: Record<SearchIntent, {
      count: number;
      percentage: number;
      avgConfidence: number;
      successRate: number;
  ...
  private extractModifiers(query: string): string[] {
    ...
    return modifiers.filter(modifier => 
      new RegExp(`\\b${modifier}\\b`, 'i').test(query)
    );
  }
  ...
  private extractUrgencyIndicators(query: string): string[] {
    ...
    return urgencyTerms.filter(term => 
  ...
  private extractTechnicalAcronyms(query: string): string[] {
    ...
    return acronyms.filter(acronym => 
      this.domainVocabulary.has(acronym.toLowerCase())
    );
  }
