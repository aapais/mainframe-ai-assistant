...
export type FailureType = 'zero_results' | 'poor_relevance' | 'timeout' | 'error' | 'incomplete_results' | 'user_abandoned' | 'refinement_loop';
...
export type FailureReasonType = 'content_gap' | 'query_complexity' | 'terminology_mismatch' | 'scope_too_broad' | 'scope_too_narrow' | 'spelling_error' | 'syntax_error' | 'index_issue' | 'system_performance' | 'user_expectation';
...
export type UserActionType = 'query_refinement' | 'filter_applied' | 'result_clicked' | 'result_rated' | 'search_abandoned' | 'help_requested' | 'feedback_provided';
export type ResolutionMethod = 'query_suggestion' | 'content_creation' | 'user_guidance' | 'system_fix' | 'manual_intervention' | 'user_education';
...
export declare class FailedSearchDetector {
    ...
    detectFailure(query: string, parsedQuery: ParsedQuery, results: SearchResult[], processingTime: number, options?: SearchOptions, userId?: string, sessionId?: string): {
        isFailed: boolean;
    ...
    markAsResolved(failureId: string, resolutionMethod: ResolutionMethod, resolutionTime?: number): void;
    generateFailureReport(timeRange?: {
        from: number;
        to: number;
    }): FailureAnalysisReport;
    getRecoverySuggestions(query: string, failureType: FailureType, context?: SearchContext): Array<{
