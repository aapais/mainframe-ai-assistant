...
export class IPAllowlistManager extends EventEmitter {
  ...
  private checkRules(ipAddress: string): IPCheckResult {
    const rules = this.getRulesForIP(ipAddress).sort(
      (a, b) => this.getRulePriority(a) - this.getRulePriority(b)
    );
  ...
  private async checkReputation(ipAddress: string): Promise<IPCheckResult> {
    ...
    if (
      ...
      reputation.riskScore > this.config.autoBlockThreshold
    ) {
      return {
        action: 'WATCH',
        riskScore: reputation.riskScore,
  ...
  private async updateActivityMetrics(ipAddress: string, context: RequestContext): Promise<void> {
    ...
    if (context.endpoint) {
      const endpoints = new Set([
        ...metrics.uniqueEndpoints.toString().split(','),
        context.endpoint,
      ]);
    ...
    metrics.averageRequestsPerHour =
      hoursSinceFirstSeen > 0 ? metrics.totalRequests / hoursSinceFirstSeen : metrics.totalRequests;
  ...
  private async addThreatReport(
    ...
    await this.auditService.logSecurityEvent('threat_report_added', report.severity, ipAddress, {
      reportId: report.id,
  ...
  private calculateReputationRiskScore(reputation: IPReputation): number {
    ...
    const avgSourceScore =
      reputation.sources.length > 0
        ? reputation.sources.reduce((sum, s) => sum + s.score, 0) / reputation.sources.length
        : 0;
    ...
    if (reputation.activityMetrics) {
      const failureRate =
        reputation.activityMetrics.failedRequests / reputation.activityMetrics.totalRequests;
  ...
  private classifyReputation(
    riskScore: number
  ): 'TRUSTED' | 'NEUTRAL' | 'SUSPICIOUS' | 'MALICIOUS' {
  ...
  private async checkReputationSource(
    ipAddress: string,
    sourceName: string
