...
class MLModelMonitor {
    ...
    async detectFeatureDrift(modelId, input, detector) {
        ...
        for (const [featureName, value] of Object.entries(features)) {
            ...
            const newStd = Math.sqrt(stats.samples.reduce((sum, val) => sum + Math.pow(val - newMean, 2), 0) / stats.samples.length);
    ...
    async detectConceptDrift(modelId, prediction, actualValue, detector) {
        ...
        const recentAccuracy = detector.accuracyWindow.reduce((sum, val) => sum + val, 0) /
    ...
    determineDriftType(featureDrift, labelDrift, conceptDrift) {
        ...
        if (conceptDrift.driftScore === maxDrift)
            return 'concept';
        if (labelDrift.driftScore === maxDrift)
            return 'label';
        return 'feature';
    }
