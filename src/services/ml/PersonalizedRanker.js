...
class PersonalizedRanker {
        ...
        interactions.forEach((interaction) => {
            ...
            userInteractions.forEach((otherInteraction) => {
                if (otherInteraction.category !== category) {
                    ...
                    cooccurrences.set(otherInteraction.category, (cooccurrences.get(otherInteraction.category) || 0) + 1);
                }
            });
        });
    ...
    async rankResults(results, query, personalizationFeatures) {
        ...
        const rankedResults = results.map(result => {
            const rankingFeatures = this.calculateRankingFeatures(result, query, userProfile, personalizationFeatures);
    ...
    calculateDiversityScore(result, searchHistory) {
        ...
        recentQueries.forEach(query => {
            ...
            const overlap = queryWords.filter(word => titleWords.includes(word) || contentWords.includes(word)).length;
    ...
    calculatePersonalizedScore(features) {
        ...
        return (features.relevanceScore * weights.relevance +
