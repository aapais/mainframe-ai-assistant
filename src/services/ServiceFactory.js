...
class ServiceFactory {
    ...
    getKnowledgeBaseService() {
        if (!this.instances.knowledgeBaseService) {
            throw new services_1.ServiceError('KnowledgeBaseService not initialized', 'SERVICE_NOT_INITIALIZED');
        }
        return this.instances.knowledgeBaseService;
    }
    getValidationService() {
        if (!this.instances.validationService) {
            throw new services_1.ServiceError('ValidationService not initialized', 'SERVICE_NOT_INITIALIZED');
        }
    ...
    getMetricsService() {
        if (!this.instances.metricsService) {
            throw new services_1.ServiceError('MetricsService not initialized', 'SERVICE_NOT_INITIALIZED');
        }
        return this.instances.metricsService;
    }
    getImportExportService() {
        if (!this.instances.importExportService) {
            throw new services_1.ServiceError('ImportExportService not initialized', 'SERVICE_NOT_INITIALIZED');
        }
    ...
    registerCustomService(serviceName, serviceInstance) {
        if (this.initialized) {
            throw new services_1.ServiceError('Cannot register services after initialization', 'ALREADY_INITIALIZED');
        }
    ...
    async doInitialize() {
        try {
            ...
            await this.close();
            throw new services_1.ServiceError(`Service initialization failed: ${error.message}`, 'INITIALIZATION_FAILED', 500, { originalError: error });
        }
    }
    async initializeService(serviceName) {
        ...
        switch (serviceName) {
            case 'knowledgeBaseService':
                this.instances.knowledgeBaseService = new KnowledgeBaseService_1.default(this.config, this.instances.validationService, this.instances.searchService, this.instances.cacheService, this.instances.metricsService, this.instances.importExportService);
            ...
            case 'cacheService':
                ...
                break;
            case 'metricsService':
                this.instances.metricsService = new MetricsService_1.default(this.config.metrics, this.config.database.path.replace('.db', '-metrics.db'));
                break;
            case 'importExportService':
                if (!this.instances.knowledgeBaseService) {
                    throw new services_1.ServiceError('KnowledgeBaseService required for ImportExportService', 'DEPENDENCY_MISSING');
                }
                this.instances.importExportService = new ImportExportService_1.default(this.instances.knowledgeBaseService, this.instances.validationService);
                break;
            default:
