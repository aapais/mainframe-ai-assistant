...
class RAGService {
    ...
    async query(query, options = {}) {
        try {
            ...
            logger.info('Iniciando query RAG', {
                query: typeof sanitizedQuery === 'string' ? sanitizedQuery.substring(0, 100) : 'complex_query',
                config: searchConfig
            });
    ...
    async generateEmbedding(text) {
        try {
            ...
            switch (this.config.embeddings.provider) {
                ...
                default:
                    throw new EmbeddingError(`Provider de embedding não suportado: ${this.config.embeddings.provider}`);
            }
    ...
    optimizeContextWindow(results, config) {
        ...
        for (const result of results) {
            ...
            if (currentLength + contentLength <= maxLength) {
                ...
                if (remainingSpace > 200) { // Mínimo útil
    ...
    generateCacheKey(query, config) {
        ...
        return `${Buffer.from(queryStr + configStr).toString('base64').substring(0, 32)}`;
    }
