...
class RAGPipeline {
  constructor(config = {}) {
    this.config = {
      ...
      documents: {
        ...
        chunkOverlap:
          config.documents?.chunkOverlap || parseInt(process.env.RAG_CHUNK_OVERLAP) || 200,
  ...
  async performRetrieval(queryEmbedding, options) {
    try {
      const searchResults = await this.vectorDatabase.similaritySearch(queryEmbedding, {
  ...
  getCacheKey(query, options) {
    ...
    return `rag_${Buffer.from(query + optionsKey)
      .toString('base64')
      .substring(0, 32)}`;
  }
