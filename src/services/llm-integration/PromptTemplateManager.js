...
class PromptTemplate {
  ...
  format(variables = {}) {
    ...
    const missingVars = this.variables.filter(
      varName => !(varName in variables) && formatted.includes(`{{${varName}}}`)
    );
...
Provide actionable insights with supporting data and strategic recommendations.`,
  ...
  getTemplateStats() {
    ...
    stats.averageVariables =
      templates.length > 0 ? Math.round((totalVariables / templates.length) * 100) / 100 : 0;
