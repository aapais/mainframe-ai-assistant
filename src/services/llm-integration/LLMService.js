...
class LLMService {
        ...
        this.config = {
            providers: {
                openai: {
                    ...
                    temperature: config.openai?.temperature || parseFloat(process.env.OPENAI_TEMPERATURE) || 0.3,
                ...
                claude: {
                    ...
                    temperature: config.claude?.temperature || parseFloat(process.env.ANTHROPIC_TEMPERATURE) || 0.3,
                ...
                azure: {
                    ...
                    deploymentName: config.azure?.deploymentName || process.env.AZURE_OPENAI_DEPLOYMENT || 'gpt-4',
                    apiVersion: config.azure?.apiVersion || process.env.AZURE_OPENAI_VERSION || '2023-12-01-preview',
                    maxTokens: config.azure?.maxTokens || parseInt(process.env.AZURE_MAX_TOKENS) || 4000,
                    temperature: config.azure?.temperature || parseFloat(process.env.AZURE_TEMPERATURE) || 0.3
                }
            },
            fallback: {
                enabled: config.fallback?.enabled ?? (process.env.LLM_ENABLE_FALLBACK === 'true'),
                primaryProvider: config.fallback?.primaryProvider || process.env.LLM_PRIMARY_PROVIDER || 'openai',
                fallbackProviders: config.fallback?.fallbackProviders ||
                    (process.env.LLM_FALLBACK_PROVIDERS ? process.env.LLM_FALLBACK_PROVIDERS.split(',') : ['claude', 'azure']),
            ...
            compliance: {
                ...
                anonymizeData: config.compliance?.anonymizeData ?? (process.env.LLM_ANONYMIZE_DATA === 'true'),
                logOperations: config.compliance?.logOperations ?? (process.env.LLM_LOG_OPERATIONS === 'true'),
            ...
            features: {
                ...
                semanticSearch: config.features?.semanticSearch ?? (process.env.LLM_ENABLE_SEMANTIC_SEARCH === 'true'),
                fraudDetection: config.features?.fraudDetection ?? (process.env.LLM_ENABLE_FRAUD_DETECTION === 'true'),
                complianceCheck: config.features?.complianceCheck ?? (process.env.LLM_ENABLE_COMPLIANCE_CHECK === 'true')
            }
        };
    ...
    async applySanitization(prompt, options) {
        ...
        if (Array.isArray(prompt)) {
            return await Promise.all(prompt.map(async msg => ({
                ...msg,
        ...
        for (const [type, providerMap] of this.metrics.entries()) {
            ...
            for (const [provider, values] of providerMap.entries()) {
                ...
                summary[type][provider] = {
                    ...
                    average: recentValues.length > 0 ?
                        recentValues.reduce((sum, v) => sum + v.value, 0) / recentValues.length : 0
                };
            }
        }
