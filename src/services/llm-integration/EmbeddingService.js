...
class EmbeddingService {
  ...
  findSimilar(queryEmbedding, candidateEmbeddings, options = {}) {
    const { topK = 5, threshold = 0.7, method = 'cosine', includeScores = true } = options;
    ...
    similarities.sort((a, b) =>
      ascending ? a.similarity - b.similarity : b.similarity - a.similarity
    );

    // Filter by threshold and limit to topK
    let results = similarities
      .filter(item => (ascending ? item.similarity <= threshold : item.similarity >= threshold))
      .slice(0, topK);
  ...
  async checkRateLimit(textCount) {
    ...
    // Clean old requests
    this.rateLimiter.requests = this.rateLimiter.requests.filter(time => now - time < windowMs);
    this.rateLimiter.tokens = this.rateLimiter.tokens.filter(time => now - time < windowMs);
