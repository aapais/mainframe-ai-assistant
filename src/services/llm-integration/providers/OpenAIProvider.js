...
class OpenAIProvider {
  ...
  handleAPIError(error, operation) {
    ...
    if (error.status === 429) {
      ...
      return new RateLimitError(`OpenAI rate limit atingido: ${error.message}`, retryAfter, {
        operation,
        status: error.status,
      });
    }
    ...
    if (error.status === 402 || error.type === 'insufficient_quota') {
      return new QuotaExceededError(`Quota OpenAI excedida: ${error.message}`, null, {
        operation,
        status: error.status,
      });
    }
    ...
    if (error.status === 401 || error.status === 403) {
      return new AuthenticationError(`Falha na autenticação OpenAI: ${error.message}`, 'openai', {
        operation,
        status: error.status,
      });
    }
    ...
    if (error.code === 'ETIMEDOUT' || error.message.includes('timeout')) {
      return new TimeoutError(`Timeout OpenAI: ${error.message}`, this.config.timeout, {
        operation,
      });
    }
    ...
    if (error.status === 400 && error.message.includes('format')) {
      return new ResponseFormatError(`Formato de resposta inválido: ${error.message}`, null, {
        operation,
        status: error.status,
      });
    }
