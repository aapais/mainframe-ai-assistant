...
function validateSearchQuery(searchQuery) {
    ...
    if (searchQuery.threshold !== undefined) {
        if (typeof searchQuery.threshold !== 'number' ||
            searchQuery.threshold < 0 || searchQuery.threshold > 1) {
            throw new Error('Threshold deve ser um número entre 0 e 1');
        }
    }

    if (searchQuery.limit !== undefined) {
        if (!Number.isInteger(searchQuery.limit) ||
            searchQuery.limit < 1 || searchQuery.limit > 100) {
...
function validateDocumentMetadata(metadata) {
    ...
    const allowedFields = [
        'title', 'category', 'source', 'source_type', 'timestamp',
        'author', 'version', 'tags', 'language', 'verified'
    ];
