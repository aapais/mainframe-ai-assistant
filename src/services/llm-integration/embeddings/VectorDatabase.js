...
class VectorDatabase {
    constructor(config = {}) {
        this.config = {
            ...
            connection: {
                ...
                persistDirectory: config.connection?.persistDirectory || process.env.CHROMA_PERSIST_DIRECTORY || './data/chroma'
            },
            collections: {
                banking_knowledge: config.collections?.banking_knowledge || process.env.CHROMA_COLLECTION_NAME || 'banking_knowledge',
            ...
            performance: {
                ...
                maxResults: config.performance?.maxResults || parseInt(process.env.CHROMA_MAX_RESULTS) || 100,
    ...
    async ensureCollections() {
        for (const [name, collectionName] of Object.entries(this.config.collections)) {
            try {
                ...
                throw new VectorDBError(`Erro ao criar collection ${name}: ${error.message}`, 'create_collection');
            }
        }
    }
    ...
    async updateDocuments(collectionName, documents) {
        try {
            ...
            throw new VectorDBError(`Falha ao atualizar documentos: ${error.message}`, 'update_documents');
        }
    }
