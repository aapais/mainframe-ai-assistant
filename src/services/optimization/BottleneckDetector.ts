/**
 * BottleneckDetector - Intelligent performance bottleneck detection and analysis
 * Identifies performance bottlenecks across system components and provides optimization recommendations
 */
...
export interface Bottleneck {
  ...
  trend: {
    ...
    prediction: string; // short description of predicted outcome
  };
}
...
export class BottleneckDetector extends EventEmitter {
  ...
  private detectionRules: Map<string, (metrics: PerformanceMetric[]) => Bottleneck[]> = new Map();
  ...
  async detectBottlenecks(): Promise<any[]> {
    ...
    // Emit events for critical bottlenecks
  ...
  async applyOptimization(recommendation: any): Promise<boolean> {
    try {
      ...
      // This would integrate with actual system management
   ...
   */
  private calculateHealthScore(health: ComponentHealth): void {
    const recentMetrics = health.metrics.slice(-10); // Last 10 metrics
    ...
    recentMetrics.forEach(metric => {
      ...
      if (threshold) {
        ...
        if (metric.value >= threshold.critical) {
          ...
          // Good performance - scale from 50-100 based on how far below warning
          const range = threshold.warning * 0.8; // Assume good performance is 80% of warning
   ...
   */
  private updateHealthStatus(health: ComponentHealth): void {
    if (health.score >= 80) {
   ...
   */
  private updateHealthTrends(health: ComponentHealth): void {
   ...
   */
  private checkImediateBottleneck(metric: PerformanceMetric): void {
   ...
   */
  private calculateSuggestionPriority(bottleneck: Bottleneck): number {
  ...
  private analyzeTrend(metrics: PerformanceMetric[]): 'improving' | 'stable' | 'degrading' {
    ...
    // For most metrics, higher values indicate degrading performance
    if (Math.abs(changePercent) < 5) return 'stable';
    return changePercent > 0 ? 'degrading' : 'improving';
  }

  private calculateVelocity(metrics: PerformanceMetric[]): number {
    ...
    return (totalChange / timeSpan) * 1000; // Change per second
  }
  ...
  private createBottleneckAnalysisMetric(bottlenecks: Bottleneck[]): any {
