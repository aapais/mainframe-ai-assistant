...
class CategoryManager {
  ...
  async classifyIncident(incident) {
    ...
    try {
      ...
      const [mlResults, nlpResults, keywordResults, patternResults] = await Promise.all([
        ...
        this.options.enableNLP
          ? this.classifyWithNLP(processedText, incident)
          : Promise.resolve([]),
        this.options.enableKeywords
          ? this.classifyWithKeywords(processedText, incident)
          : Promise.resolve([]),
        this.options.enablePatterns
          ? this.classifyWithPatterns(processedText, incident)
          : Promise.resolve([]),
      ]);
  ...
  combineResults(methodResults) {
    ...
    const results = Array.from(combinedMap.values()).map(item => {
      ...
      const maxPossibleScore = Object.values(this.options.hybridWeights).reduce(
