...
class TechnologyClassifier {
        ...
        for (const [categoryId, model] of Object.entries(this.classificationModels)) {
            ...
            if (score > 0.3) {
                results.push({
                    category: categoryId,
                    confidence: score,
                    matches: model.indicators.filter(indicator =>
    ...
    combineClassificationResults(results) {
        ...
        ['keywords', 'patterns', 'nlp', 'contextual'].forEach(method => {
            ...
            const weight = this.config[`weight${method.charAt(0).toUpperCase() + method.slice(1)}`] || 0.25;
    ...
    updateMetrics(classification) {
        ...
        const totalConfidence = this.metrics.averageConfidence * (this.metrics.totalClassifications - 1) + classification.confidence;
