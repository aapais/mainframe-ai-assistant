...
class MLClassifier {
  ...
  async formatPredictionResults(probabilities, originalText, options) {
    ...
    for (let i = 0; i < classes.length; i++) {
      ...
      if (confidence > 0.1) {
        // Filtra resultados muito baixos
        results.push({
          category: classes[i],
          confidence: confidence,
          features: options.includeFeatures
            ? await this.extractFeatures(this.preprocessText(originalText))
            : undefined,
  ...
  generateExplanation(text, category, confidence) {
    ...
    if (foundKeywords.length > 0) {
      ...
      explanation.reasoning.push(
        `Encontradas palavras-chave relevantes: ${foundKeywords.join(', ')}`
      );
    }
...
class TFIDFVectorizer {
  ...
  async fit(documents) {
    ...
    const filteredTerms = Array.from(allTerms).filter(term => {
      ...
      return docFreq >= this.minDocumentFreq / documents.length && docFreq <= this.maxDocumentFreq;
    });
...
class FeatureSelector {
  ...
  async fitTransform(features, labels) {
    ...
    // Aplica seleção
    return features.map(sample => this.selectedFeatures.map(index => sample[index]));
  }
  ...
  async transform(features) {
    ...
    return features.map(sample => this.selectedFeatures.map(index => sample[index] || 0));
  }
}
...
class MultinomialNaiveBayes {
  ...
  async fit(features, labels) {
    ...
    for (const cls of this.classes) {
      ...
      const classFeatureProb = featureSums.map(
        sum => (sum + this.alpha) / (totalSum + this.alpha * numFeatures)
      );
  ...
  setState(state) {
    ...
    this.featureProb = new Map(
      Object.entries(state.featureProb).map(([k, v]) => [
        k,
