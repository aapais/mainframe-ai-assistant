...
exports.getCacheApiClient = exports.createCacheApiClient = exports.CacheApiClient = exports.CacheApiError = void 0;
...
class CacheApiClient {
    ...
    async getSuggestions(query, options) {
        ...
        return this.request('GET', `/suggestions?${params}`, options?.context ? { context: options.context } : undefined);
    }
    async getStats() {
        ...
        for (let attempt = 0; attempt <= this.retries; attempt++) {
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new CacheApiError(errorData.message || `HTTP ${response.status}`, errorData.code || 'HTTP_ERROR', response.status, errorData);
                }
                if (method === 'HEAD') {
                    return {
                        success: true,
