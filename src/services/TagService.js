...
class TagService extends events_1.EventEmitter {
  ...
  async createTag(data, userId) {
    try {
      const validatedData =
        HierarchicalCategories_schema_1.HierarchicalSchemaValidator.validateCreateTag(data);
  ...
  async bulkOperation(operation, userId) {
    try {
      const validatedOperation =
        HierarchicalCategories_schema_1.HierarchicalSchemaValidator.validateBulkTagOperation(
          operation
        );
