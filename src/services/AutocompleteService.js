  ...
  async getSuggestions(query, context) {
    try {
      const validatedQuery =
        HierarchicalCategories_schema_1.HierarchicalSchemaValidator.validateAutocompleteQuery(
          query
        );
  ...
  async aggregateSuggestions(query, context) {
    ...
    if (query.types.includes('category')) {
      const categoryRows = this.preparedStatements
        ...
        .all(queryPattern, prefixPattern, queryPattern, query.limit);
    ...
    if (query.types.includes('tag')) {
      const tagRows = this.preparedStatements
        .get('tagSuggestions')
        .all(queryPattern, queryPattern, queryPattern, query.limit);
    ...
    if (query.types.includes('entry')) {
      const entryRows = this.preparedStatements
        ...
        .all(queryPattern, queryPattern, query.limit);
    ...
    if (this.config.enableContextAware && context?.currentEntryId) {
      const contextRows = this.preparedStatements
        ...
        .all(context.currentEntryId, queryPattern, queryPattern, query.limit);
