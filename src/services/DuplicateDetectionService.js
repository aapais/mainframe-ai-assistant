...
class DuplicateDetectionService {
    ...
    async findSimilar(targetEntry, allEntries, options = {}) {
        ...
        const { threshold = 0.7, useAI = false, maxComparisons = 1000, includeFields = ['title', 'problem', 'solution', 'tags', 'category'], optimizationLevel = 'balanced' } = options;
        ...
        const filteredEntries = this.optimizeEntryList(allEntries.filter(entry => entry.id !== targetEntry.id), targetEntry, optimizationLevel).slice(0, comparisons);
    ...
    async calculateSimilarity(entry1, entry2, includeFields, useAI) {
        ...
        if (includeFields.includes('title') && entry1.title && entry2.title) {
            scores.title = Math.max(SimilarityAlgorithms.jaccard(entry1.title, entry2.title), SimilarityAlgorithms.levenshtein(entry1.title, entry2.title), SimilarityAlgorithms.ngram(entry1.title, entry2.title));
            if (scores.title > 0.7)
                matchedFields.push('title');
        }
        if (includeFields.includes('problem') && entry1.problem && entry2.problem) {
            scores.problem = Math.max(SimilarityAlgorithms.jaccard(entry1.problem, entry2.problem), SimilarityAlgorithms.cosine(entry1.problem, entry2.problem));
            if (scores.problem > 0.6)
                matchedFields.push('problem');
        }
        if (includeFields.includes('solution') && entry1.solution && entry2.solution) {
            scores.solution = Math.max(SimilarityAlgorithms.jaccard(entry1.solution, entry2.solution), SimilarityAlgorithms.cosine(entry1.solution, entry2.solution));
            if (scores.solution > 0.6)
            ...
            default:
                ...
                const similarTags = entries.filter(entry => {
                    if (!entry.tags || !targetEntry.tags)
                        return false;
                    const commonTags = entry.tags.filter(tag => targetEntry.tags.some(tTag => tTag.toLowerCase().includes(tag.toLowerCase())));
    ...
    suggestMergeStrategy(primary, duplicates) {
        ...
        const maxDuplicateDate = Math.max(...duplicates.map(d => new Date(d.updated_at || d.created_at || 0).getTime()));
