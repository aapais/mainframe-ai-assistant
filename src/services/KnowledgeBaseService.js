...
class KnowledgeBaseService extends events_1.EventEmitter {
    ...
    startTime = Date.now();
    statements;
    constructor(config = services_1.DEFAULT_SERVICE_CONFIG, validationService, searchService, cacheService, metricsService, importExportService) {
        super();
        this.config = config;
    ...
    async initialize() {
        try {
            ...
            const serviceError = new services_1.DatabaseError('Failed to initialize KnowledgeBaseService', 'initialize', { originalError: error });
    ...
    async create(entry) {
        ...
        if (this.validationService) {
            ...
            if (!validation.valid) {
                throw new services_1.ValidationError(`Entry validation failed: ${validation.errors.map(e => e.message).join(', ')}`, validation.errors[0]?.field || 'unknown');
            }
        }
        ...
        const transaction = this.db.transaction(() => {
            try {
                this.statements.insertEntry.run(id, entry.title, entry.problem, entry.solution, entry.category, now.toISOString(), now.toISOString(), entry.created_by || 'system', 0, 0, 0, 1);
                ...
                throw new services_1.DatabaseError(`Failed to create entry: ${error.message}`, 'create', { entry, originalError: error });
            }
        });
    ...
    async createBatch(entries) {
        ...
        if (this.validationService) {
            ...
            if (invalidEntries.length > 0) {
                throw new services_1.ValidationError(`Batch validation failed for ${invalidEntries.length} entries`, 'batch');
            }
        }
        const ids = [];
        const now = new Date();
    ...
    async read(id) {
      ...
      `).all(id);
            ...
            throw new services_1.DatabaseError(`Failed to read entry ${id}: ${error.message}`, 'read', { id, originalError: error });
        }
    }
    async readBatch(ids) {
        this.ensureInitialized();
    ...
    async update(id, updates) {
        ...
        if (this.validationService) {
            ...
            if (!validation.valid) {
                throw new services_1.ValidationError(`Update validation failed: ${validation.errors.map(e => e.message).join(', ')}`, validation.errors[0]?.field || 'unknown');
            }
        }
        ...
        `;
                ...
                throw new services_1.DatabaseError(`Failed to update entry ${id}: ${error.message}`, 'update', { id, updates, originalError: error });
            }
        });
    ...
    async updateBatch(updates) {
        ...
        if (successfulUpdates.length > 0) {
            ...
            this.emit('entries:batch-updated', successfulUpdates.map((u, i) => ({
                id: u.id,
                entry: updatedEntries[i],
                changes: u.updates
            })));
            if (this.metricsService) {
                await Promise.all(successfulUpdates.map(u => this.metricsService.recordUsage(u.id, 'update')));
            }
        }
        return results;
    }
    async delete(id) {
        ...
        const transaction = this.db.transaction(() => {
            try {
                ...
                throw new services_1.DatabaseError(`Failed to delete entry ${id}: ${error.message}`, 'delete', { id, originalError: error });
            }
        });
        const success = transaction();
        if (success) {
    ...
    async deleteBatch(ids) {
        ...
        if (successfulDeletions.length > 0) {
            ...
            if (this.metricsService) {
                await Promise.all(successfulDeletions.map(id => this.metricsService.recordUsage(id, 'delete')));
            }
        }
        return results;
    }
    async list(options = {}) {
        this.ensureInitialized();
        const { limit = 50, offset = 0, category, sortBy = 'created_at', sortOrder = 'desc', includeMetrics = false, filters = {} } = options;
        const conditions = [];
        const params = [];
        if (category) {
      ...
      `;
        ...
        `;
            ...
            throw new services_1.DatabaseError(`Failed to list entries: ${error.message}`, 'list', { options, originalError: error });
        }
    }
    async search(query, options = {}) {
        ...
        try {
            ...
            const searchError = new services_1.ServiceError(`Search failed: ${error.message}`, 'SEARCH_FAILED', 500, { query, options, originalError: error });
    ...
    async recordUsage(id, successful, userId) {
        ...
        try {
            ...
            throw new services_1.DatabaseError(`Failed to record usage for entry ${id}: ${error.message}`, 'recordUsage', { id, successful, userId, originalError: error });
        }
    }
    async getMetrics() {
        this.ensureInitialized();
      ...
      `).all();
            ...
            throw new services_1.DatabaseError(`Failed to get metrics: ${error.message}`, 'getMetrics', { originalError: error });
        }
    }
    ...
    async backup() {
        ...
        try {
            ...
            const backupPath = path_1.default.join(this.config.database.backup.path || './backups', `kb-backup-${timestamp}.db`);
            ...
            throw new services_1.ServiceError(`Backup failed: ${error.message}`, 'BACKUP_FAILED', 500, { originalError: error });
        }
    }
    async restore(backupPath) {
        ...
        try {
            ...
            throw new services_1.ServiceError(`Restore failed: ${error.message}`, 'RESTORE_FAILED', 500, { backupPath, originalError: error });
        }
    }
    async close() {
        try {
    ...
    async initializeDatabase() {
        try {
            ...
            throw new services_1.DatabaseError(`Failed to initialize database: ${error.message}`, 'initialize', { path: this.config.database.path, originalError: error });
        }
    }
    async createSchema() {
        const schema = `
    ...
    updateFTSIndex(id, entry) {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(id, entry.title || '', entry.problem || '', entry.solution || '', tags, entry.category || '');
    }
