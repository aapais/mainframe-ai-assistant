...
class SearchMetrics extends events_1.EventEmitter {
    ...
    async exportMetrics(format, timeRange = '24h') {
        ...
        if (format === 'json') {
            ...
            const rows = allMetrics.map(m => `${m.timestamp},${m.operation},${m.duration},${m.success},${JSON.stringify(m.metadata || {})}`).join('\n');
            return headers + rows;
        }
    }
    initializeMetrics() {
        ...
        `);
                const transaction = this.database.transaction(() => {
                    this.metricsBuffer.forEach(metric => {
                        stmt.run(metric.timestamp, metric.operation, metric.duration, metric.success, metric.metadata ? JSON.stringify(metric.metadata) : null);
                    });
                });
                transaction();
            }
    ...
    assessSystemHealth(avgResponseTime, cacheHitRate, alertCount) {
        ...
        if (avgResponseTime > 1000 || cacheHitRate < 0.5)
            return 'critical';
        if (avgResponseTime > 500 || cacheHitRate < 0.7)
            return 'warning';
        if (avgResponseTime < 200 && cacheHitRate > 0.9)
            return 'excellent';
        return 'good';
    }
