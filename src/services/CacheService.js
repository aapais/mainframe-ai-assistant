...
class CacheService {
  ...
  validateKey(key) {
    ...
    if (key.length > 250) {
      throw new services_1.CacheError('Key too long (max 250 characters)', 'validation', {
        key,
        length: key.length,
      });
    }
  ...
  validateValue(value) {
    ...
    try {
      ...
      throw new services_1.CacheError('Value contains circular references', 'validation', {
        error: error.message,
      });
    }
  }
