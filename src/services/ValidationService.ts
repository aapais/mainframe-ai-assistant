 ...
 * Production-ready validation with custom rules and security features
 */
 ...
 * Provides comprehensive validation and sanitization for all KB operations
 */
export class ValidationService implements IValidationService {
  ...
  validateBatch(entries: KBEntryInput[]): ValidationResult[] {
    if (!Array.isArray(entries)) {
      throw new ServiceError('Entries must be an array', 'INVALID_INPUT');
    }
  ...
  /**
   * Sanitize multiple entries at once
   */
  ...
  /**
   * Remove custom validator for a field
   */
  ...
  private performContentQualityChecks(
    ...
    // Check for common mainframe terms
  ...
  private performSecurityChecks(
    ...
    const checkText = (text: string, fieldName: string) => {
      ...
      // Check for URLs that might expose sensitive info
    ...
    if (entry.problem) checkText(entry.problem, 'problem');
    if (entry.solution) checkText(entry.solution, 'solution');
    if (entry.title) checkText(entry.title, 'title');
  }
  ...
  private calculateQualityScore(
    ...
    score += Math.min(termCount * 2, 10); // Up to 10 bonus points
