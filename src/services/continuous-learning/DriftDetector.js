...
class DriftDetector extends events_1.EventEmitter {
  ...
  updateCurrentWindow(features) {
    for (const [featureName, values] of features) {
      ...
      if (
        !this.referenceData.has(featureName) &&
        currentData.length >= this.config.minSamplesForTest
      ) {
        this.referenceData.set(featureName, [
          ...currentData.slice(0, Math.floor(currentData.length / 2)),
        ]);
      }
    }
  }
  ...
  async detectFeatureDrift(featureName, currentValues) {
    ...
    const significantTests = testResults.filter(
      result => result.pValue < this.config.alertThreshold
    );
  ...
  chiSquareTest(reference, current) {
    ...
    for (const category of allCategories) {
      ...
      if (expected > 5) {
        // Regra mínima para qui-quadrado
  ...
  async handleDriftDetection(driftEvent) {
    ...
    logger.warn(
      `Drift detectado: ${driftEvent.type} - Score: ${driftEvent.driftScore.toFixed(3)} - Features: ${driftEvent.affectedFeatures.join(', ')}`
    );
  ...
  async getCurrentStatus() {
    ...
    return {
      ...
      lastDriftDetected:
        recentDrifts.length > 0 ? recentDrifts[recentDrifts.length - 1].timestamp : null,
    };
  }
