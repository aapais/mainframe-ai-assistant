...
class MetricsCollector extends events_1.EventEmitter {
  ...
  calculateBatchMetrics(feedbackBatch) {
    ...
    return {
      ...
      userSatisfaction:
        metrics.userSatisfaction.count > 0
          ? metrics.userSatisfaction.total / metrics.userSatisfaction.count
          : 0,
      usefulness:
        metrics.usefulness.count > 0 ? metrics.usefulness.total / metrics.usefulness.count : 0,
      relevance:
        metrics.relevance.count > 0 ? metrics.relevance.total / metrics.relevance.count : 0,
      averageResponseTime:
        metrics.responseTime.count > 0
          ? metrics.responseTime.total / metrics.responseTime.count
          : 0,
      errorRate:
        metrics.errorRate.total > 0 ? metrics.errorRate.errors / metrics.errorRate.total : 0,
      qualityScore:
        metrics.qualityScore.count > 0
          ? metrics.qualityScore.total / metrics.qualityScore.count
          : 0,
      segmentation: {
  ...
  calculateSystemReliability(metrics) {
    ...
    const responseTimeFactor =
      metrics.averageResponseTime > 0 ? Math.max(0, 1 - metrics.averageResponseTime / 5000) : 1; // 5s como referência
  ...
  async updateTrends(batchMetrics) {
    ...
    for (const metricName of metricNames) {
      ...
      const value =
        metricName === 'feedbackVolume' ? batchMetrics.size : batchMetrics.metrics[metricName];
  ...
  async getCurrentPerformance() {
    ...
    return {
      ...
      userSatisfaction:
        realTime.feedbackCount > 0 ? realTime.totalRating / realTime.feedbackCount : 0,
      averageResponseTime:
        realTime.feedbackCount > 0 ? realTime.totalResponseTime / realTime.feedbackCount : 0,
      errorRate: realTime.feedbackCount > 0 ? realTime.errors / realTime.feedbackCount : 0,
  ...
  getSegmentationAnalysis() {
    ...
    if (segmentation.bySuggestionType) {
      analysis.suggestionTypes = Object.entries(segmentation.bySuggestionType).map(
        ([type, data]) => ({
          type,
          count: data.count,
          averageRating: data.count > 0 ? data.totalRating / data.count : 0,
  ...
  getBenchmarkComparison() {
    ...
    const currentPerformance = {
      ...
      responseTime:
        current.feedbackCount > 0 ? current.totalResponseTime / current.feedbackCount : 0,
