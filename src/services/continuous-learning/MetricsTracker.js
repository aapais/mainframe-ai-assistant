...
class MetricsTracker extends EventEmitter {
  ...
  async exportMetrics(query, format = 'json') {
    ...
    for (const name of names) {
      ...
      if (includeRaw) {
        exportData.metrics[name].raw = await this.getMetricHistory(name, tags, startTime, endTime);
      }
  ...
  checkAlerts(metric) {
    ...
    for (const alert of this.alertStates.values()) {
      ...
      // Verificar cooldown
      if (alert.lastTriggered && metric.timestamp - alert.lastTriggered < alert.cooldownPeriod) {
        continue;
      }
  ...
  async performAggregation(level, startTime = null, endTime = null) {
    try {
      ...
      logger.debug(
        `Executando agregação ${level}: ${startTime.toISOString()} - ${endTime.toISOString()}`
      );
