...
class FeedbackCollector extends EventEmitter {
  ...
  async collectFeedback(startTime, endTime) {
    try {
      logger.info(
        `Coletando feedback do período: ${startTime.toISOString()} - ${endTime.toISOString()}`
      );
  ...
  recordABTestMetric(testId, group, incident, metrics) {
    ...
    if (metrics.wasSuccessful) {
      groupMetrics.successRate =
        (groupMetrics.successRate * (groupMetrics.samples - 1) + 1) / groupMetrics.samples;
    } else {
      groupMetrics.successRate =
        (groupMetrics.successRate * (groupMetrics.samples - 1)) / groupMetrics.samples;
    }

    if (metrics.satisfaction) {
      groupMetrics.avgSatisfaction =
        (groupMetrics.avgSatisfaction * (groupMetrics.samples - 1) + metrics.satisfaction) /
        groupMetrics.samples;
    }
  ...
  updateRealtimeMetrics(entry) {
    ...
    if (entry.type === 'operator') {
      const currentSuccess =
        this.realtimeMetrics.successRate * (this.realtimeMetrics.totalFeedbacks - 1);
      this.realtimeMetrics.successRate =
        (currentSuccess + (entry.wasSuccessful ? 1 : 0)) / this.realtimeMetrics.totalFeedbacks;

      const currentTime =
        this.realtimeMetrics.avgResolutionTime * (this.realtimeMetrics.totalFeedbacks - 1);
      this.realtimeMetrics.avgResolutionTime =
        (currentTime + entry.timeToResolve) / this.realtimeMetrics.totalFeedbacks;
    }

    if (entry.type === 'user_satisfaction') {
      const currentSat =
        this.realtimeMetrics.averageSatisfaction * (this.realtimeMetrics.totalFeedbacks - 1);
      this.realtimeMetrics.averageSatisfaction =
        (currentSat + entry.overallRating) / this.realtimeMetrics.totalFeedbacks;
    }
  }
