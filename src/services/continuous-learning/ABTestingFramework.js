...
class ABTestingFramework extends EventEmitter {
    ...
    try {
      ...
      for (const [metricName, value] of Object.entries(metrics)) {
        ...
        if (metric.count > 1) {
          ...
          metric.variance =
            ((metric.count - 2) * metric.variance + delta * delta) / (metric.count - 1);
        }
      }
  ...
  async analyzeTest(testId) {
    try {
      ...
      logger.info(
        `Análise concluída para teste ${testId} - Significância: ${analysis.overallSignificance?.toFixed(4)}`
      );
  ...
  async finalizeTest(testId, reason = 'completed') {
    try {
      ...
      this.metrics.avgTestDuration =
        (this.metrics.avgTestDuration * (this.metrics.totalTests - 1) + duration) /
        this.metrics.totalTests;
  ...
  async analyzeContinuousMetric(controlData, testData) {
    ...
    const df =
      Math.pow(controlVar / controlN + testVar / testN, 2) /
    ...
    const confidenceInterval = [difference - marginOfError, difference + marginOfError];

    // Effect size (Cohen's d)
    const pooledStd = Math.sqrt(
      ((controlN - 1) * controlVar + (testN - 1) * testVar) / (controlN + testN - 2)
    );
  ...
  async analyzeProportionMetric(controlData, testData) {
    ...
    const marginOfError =
      zCritical *
      Math.sqrt((controlProp * (1 - controlProp)) / controlN + (testProp * (1 - testProp)) / testN);
    const confidenceInterval = [difference - marginOfError, difference + marginOfError];
  ...
  validateTestConfig(config) {
    ...
    if (
      config.trafficSplit &&
      (config.trafficSplit <= 0 || config.trafficSplit > this.config.maxTrafficSplit)
    ) {
  ...
  async checkTrafficConflicts(trafficSplit) {
    const currentAllocation = Array.from(this.state.trafficAllocations.values()).reduce(
  ...
  shouldRunIntermediateAnalysis(test) {
    ...
    return totalSamples >= test.minimumSampleSize && totalSamples % 100 === 0;
  }
  ...
  checkEarlyStoppingCriteria(test, analysis) {
    ...
    if (
      analysis.overallSignificance &&
      analysis.overallSignificance < test.significanceLevel * 0.1
    ) {
      // 10x mais rigoroso
      return {
        shouldStop: true,
  ...
  async checkGuardRails(test, analysis) {
    ...
    for (const metric of criticalMetrics) {
      const metricAnalysis = analysis.metrics[metric];
      if (metricAnalysis && metricAnalysis.relativeImprovement < -0.05) {
        // 5% de degradação
        test.alerts.push({
        ...
        if (metricAnalysis.relativeImprovement < -0.1) {
          // 10% = parada crítica
  ...
  identifyLimitations(test, analysis) {
    ...
    if (duration < 7 * 24 * 60 * 60 * 1000) {
      // menos de 7 dias
      limitations.push(
        'Duração do teste pode ser insuficiente para capturar variabilidade semanal'
      );
    }
