...
class LearningPipeline extends events_1.EventEmitter {
  ...
  shouldRetrain(feedbackResults, patternResults) {
    ...
    const hasNewPatterns =
      patternResults.newIncidentTypes.length > 0 || patternResults.behaviorChanges.length > 0;
    const hasLowConfidence =
      feedbackResults.resolutionMetrics.averageConfidence < this.config.confidenceThreshold;
  ...
  async validateModels(retrainingResults) {
    ...
    return {
      isValid: validation.accuracy > 0.85 && validation.precision > 0.8,
      metrics: validation,
      recommendation:
        validation.accuracy > 0.9 ? 'deploy' : validation.accuracy > 0.85 ? 'ab_test' : 'reject',
    };
  }
  ...
  async handleExperimentCompletion(result) {
    logger.info(
      `Experimento completado: ${result.experimentId} - Resultado: ${result.winner || 'inconclusivo'}`
    );
  ...
  async handleRetrainingCompletion(result) {
    logger.info(
      `Retreino completado - Modelo: ${result.modelId} - Performance: ${result.accuracy}`
    );
  ...
  getStatus() {
    return {
      ...
      nextCycleIn: this.learningTimer ? new Date(Date.now() + this.config.learningCycle) : null,
    };
  }
