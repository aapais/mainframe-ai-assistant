...
class ModelRetrainer extends EventEmitter {
  ...
  updateMetrics(results, duration) {
    ...
    this.metrics.avgTrainingTime =
      (currentAvg * (this.metrics.totalRetrains - 1) + duration) / this.metrics.totalRetrains;
  }
  ...
  calculateOverallValidation(individualValidations) {
    ...
    return {
      ...
      overallAccuracy: this.calculateMean(
        Object.values(individualValidations).map(v => v.metrics.accuracy)
      ),
    };
  }
