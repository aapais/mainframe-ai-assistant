...
class ABTestingManager extends events_1.EventEmitter {
    ...
    for (const feedback of feedbackData) {
      if (feedback.metadata && feedback.metadata.sessionId) {
        ...
        const assignment = Array.from(this.assignments.values()).find(
          a =>
            a.experimentId === experimentId && a.context.sessionId === feedback.metadata.sessionId
        );
  ...
  async finalizeExperiment(experimentId, reason = 'manual') {
    ...
    logger.info(
      `Experimento finalizado: ${experimentId} - Razão: ${reason} - Vencedor: ${winner?.variantId || 'nenhum'}`
    );
  ...
  async calculateResults(experimentId) {
    ...
    for (const variant of experiment.variants) {
      ...
      const averageValue =
        outcomes.length > 0 ? outcomes.reduce((sum, o) => sum + o.value, 0) / outcomes.length : 0;
    ...
    return {
      ...
      improvement:
        winner.conversionRate /
          results.variants.find(v => v.variantId !== winner.variantId).conversionRate -
        1,
  ...
  async getDashboard() {
    ...
    const summary = {
      ...
      totalParticipants: Array.from(this.experiments.values()).reduce(
        (sum, exp) => sum + exp.metrics.totalParticipants,
        0
      ),
    };
