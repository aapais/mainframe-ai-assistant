...
describe('SearchService Unit Tests', () => {
  ...
  describe('Ranking Logic Tests', () => {
    ...
    test('should rank title matches higher than content matches', async () => {
      ...
      const titleMatch = results.find(
        r => r.entry.title.toLowerCase().includes('vsam') && r.entry.id === '1'
      );
  ...
  describe('Search Options Tests', () => {
    ...
    test('should sort by recent when specified', async () => {
      ...
      if (results.length > 1) {
        for (let i = 1; i < results.length; i++) {
          expect(results[i - 1].entry.created_at.getTime()).toBeGreaterThanOrEqual(
            results[i].entry.created_at.getTime()
          );
        }
      }
    });
  ...
  describe('Performance Validation Tests', () => {
    ...
    test('should handle large query strings efficiently', async () => {
      const longQuery =
        'very long query string with many words that should still be processed efficiently without timeout issues in the search algorithm';
