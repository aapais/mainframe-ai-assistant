...
describe('SearchService Quality Tests', () => {
  ...
  function createQualityTestDataset(): KBEntry[] {
    return [
      // High relevance entries for specific queries
      {
        ...
        problem: 'Job abends with VSAM status code 35, indicating the VSAM file cannot be found or opened.',
      ...
      },
      {
        ...
        problem: 'Program terminates with S0C7 abend during arithmetic operations or data manipulation.',
      ...
      },
      {
        ...
        problem: 'JCL job fails with IEF212I message indicating dataset not found during allocation.',
      ...
      },
      {
        ...
        solution: 'Check user permissions, verify resource profiles, contact security administrator.',
        category: 'System',
  ...
  describe('Relevance Scoring Quality Tests', () => {
    test('should prioritize exact title matches', async () => {
      const results = await searchService.search('VSAM Status 35 File Not Found', qualityTestEntries);
  ...
  describe('Ranking Accuracy Tests', () => {
    ...
    test('should rank by different criteria when specified', async () => {
      const relevanceResults = await searchService.search('error', qualityTestEntries, { sortBy: 'relevance' });
      const usageResults = await searchService.search('error', qualityTestEntries, { sortBy: 'usage' });
      const recentResults = await searchService.search('error', qualityTestEntries, { sortBy: 'recent' });
      
      // Usage-sorted should be ordered by usage count
      if (usageResults.length > 1) {
        for (let i = 1; i < usageResults.length; i++) {
          expect(usageResults[i - 1].entry.usage_count)
            .toBeGreaterThanOrEqual(usageResults[i].entry.usage_count);
        }
      }
      ...
      if (recentResults.length > 1) {
        for (let i = 1; i < recentResults.length; i++) {
          expect(recentResults[i - 1].entry.created_at.getTime())
            .toBeGreaterThanOrEqual(recentResults[i].entry.created_at.getTime());
        }
      }
    });
  });
  ...
  describe('Search Context Quality Tests', () => {
    ...
    test('should recognize mainframe-specific terminology', async () => {
      ...
      for (const term of mainframeTerms) {
        ...
        const relevantEntries = results.filter(r => 
  ...
  describe('Edge Case Quality Tests', () => {
    ...
    test('should handle very long queries efficiently', async () => {
      const longQuery = 'I am having a problem with my mainframe job that is failing with a VSAM status code 35 error and I cannot figure out why the dataset is not being found even though I have checked the catalog and the DD statement looks correct and I have verified the permissions but the job still abends';
  ...
  describe('Search Quality Metrics Tests', () => {
    ...
    test('should provide appropriate match type classifications', async () => {
      ...
      for (const testCase of testCases) {
        ...
        if (results.length > 0) {
          const topResult = results[0];
          expect(['exact', 'fuzzy', 'tag', 'category', 'semantic', 'ai'])
            .toContain(topResult.matchType);
        }
      }
    });
    ...
    test('should track search quality over time', async () => {
      ...
      const avgResultCount = searchResults.reduce((sum, s) => sum + s.results.length, 0) / totalSearches;
      const avgTopScore = searchResults
