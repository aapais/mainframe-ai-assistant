...
class AuditSchema {
    ...
    static get llmInteractionSchema() {
        return this.baseAuditEntry.concat(Joi.object({
            ...
            compliance: Joi.object({
                ...
                regulatoryImpact: Joi.array().items(Joi.string().valid('SOX', 'BACEN', 'LGPD')).required()
            }).required()
        }));
    }
    ...
    static get operatorActionSchema() {
        return this.baseAuditEntry.concat(Joi.object({
            ...
            compliance: Joi.object({
                ...
                regulatoryCategory: Joi.string().valid('FINANCIAL', 'PERSONAL_DATA', 'OPERATIONAL').required()
            }).required()
        }));
    }
    ...
    static get slaMetricsSchema() {
        return this.baseAuditEntry.concat(Joi.object({
            slaType: Joi.string().valid('RESPONSE_TIME', 'RESOLUTION_TIME', 'ESCALATION_TIME', 'AVAILABILITY').required(),
    ...
    static get retentionPolicySchema() {
        return Joi.object({
            dataClassification: Joi.string().valid('FINANCIAL', 'PERSONAL_DATA', 'OPERATIONAL', 'SYSTEM', 'DEBUG').required(),
     ...
     */
    static sanitizeForLogging(data, sensitiveFields = ['password', 'token', 'secret', 'key', 'cpf', 'account']) {
