...
class RetentionManager extends EventEmitter {
    ...
    async applyRetentionPolicy(filePath, dataClassification, fileDate) {
        ...
        const retentionDays = this.config.retentionPolicies[dataClassification] || this.config.retentionPolicies.OPERATIONAL;
    ...
    generateRetentionReport() {
        ...
        if (report.archives.totalSize > 10 * 1024 * 1024 * 1024) { // 10GB
            report.recommendations.push({
                type: 'STORAGE',
                priority: 'HIGH',
    ...
    calculateRetentionExpiry(dataClassification) {
        const retentionDays = this.config.retentionPolicies[dataClassification] ||
