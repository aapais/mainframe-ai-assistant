...
class AuditService extends EventEmitter {
  ...
  affectsFinancialReporting(data) {
    return data.impact?.businessImpact === 'HIGH' || data.category === 'FINANCIAL_REPORTING';
  }
  ...
  requiresComplianceLogging(entry) {
    return (
      entry.compliance &&
      (entry.compliance.regulatoryImpact?.length > 0 || entry.compliance.requiresApproval)
    );
  }
