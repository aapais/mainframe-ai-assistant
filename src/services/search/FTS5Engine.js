...
class FTS5Engine {
    ...
    constructor(db, config = {}, tokenizerConfig = {}) {
        ...
        this.tokenizerConfig = this.mergeTokenizerConfig(FTS5Engine.DEFAULT_TOKENIZER_CONFIG, tokenizerConfig);
    }
    async initialize() {
    ...
    async addDocument(entry) {
      ...
      `);
            ...
            stmt.run(entry.id, entry.id, entry.title, entry.problem, entry.solution, entry.category, tagsString);
        }
        catch (error) {
    ...
    async registerCustomFunctions() {
        this.db.function('bm25_score', { deterministic: true }, (tf, docLen, avgDocLen, docCount, termCount) => {
        ...
        this.db.function('generate_snippet', { deterministic: true }, (text, query, maxLength = this.config.snippet.maxLength) => {
    ...
    `);
        const transaction = this.db.transaction(() => {
            entries.forEach(entry => {
                insertStmt.run(entry.id, entry.title, entry.problem, entry.solution, entry.category, entry.tags || '');
            });
        });
        transaction();
    ...
    highlightText(text, query) {
        ...
        queryTerms.forEach(term => {
            const regex = new RegExp(`(${term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, this.config.highlight.caseSensitive ? 'g' : 'gi');
            highlightedText = highlightedText.replace(regex, `${this.config.highlight.startTag}$1${this.config.highlight.endTag}`);
        });
    ...
    async logInitializationStats() {
        const stats = this.getStats();
        console.log(`📊 FTS5 Engine ready: ${stats.documentCount} indexed documents, ` +
