...
exports.FTS5Integration = void 0;
const tslib_1 = require("tslib");
const FTS5Engine_1 = tslib_1.__importDefault(require("./FTS5Engine"));
class FTS5Integration {
    db;
    fts5Engine;
    legacyEngine;
    config;
    ...
    constructor(db, legacyEngine, fts5Config = {}, integrationConfig = {}) {
        this.db = db;
        this.legacyEngine = legacyEngine;
        this.config = this.mergeConfig(FTS5Integration.DEFAULT_CONFIG, integrationConfig);
        this.fts5Engine = new FTS5Engine_1.default(db, fts5Config);
    }
    async initialize() {
    ...
    async search(query, options = {}) {
        ...
        try {
            ...
            if (this.config.performance.enableMonitoring) {
                console.log(`🔍 Search: "${query}" | Strategy: ${strategy.strategy} | ` +
    ...
    async autoComplete(prefix, limit = 5) {
        ...
        try {
            ...
            const suggestions = this.mergeAutoCompleteResults(fts5Results.status === 'fulfilled' ? fts5Results.value : [], legacyResults.status === 'fulfilled' ? legacyResults.value : [], limit);
    ...
    getPerformanceMetrics() {
        const calculateMetrics = (strategy) => {
            ...
            return {
                ...
                successRate: times.length > 0 ? 1.0 : 0.0
            };
        };
    ...
    async executeSearch(query, options, strategy) {
    ...
    async executeFTS5Search(query, options) {
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
    ...
    async executeHybridSearch(query, options) {
        ...
        const merged = this.mergeSearchResults(fts5Results.status === 'fulfilled' ? fts5Results.value : [], legacyResults.status === 'fulfilled' ? legacyResults.value : [], limit);
        return merged;
    }
    mergeSearchResults(fts5Results, legacyResults, limit) {
    ...
    async getFTS5AutoComplete(prefix, limit) {
    ...
    mergeAutoCompleteResults(fts5Results, legacyResults, limit) {
    ...
    generateCacheKey(query, options, strategy) {
        const keyParts = [
            strategy,
            query.toLowerCase(),
            options.category || '',
    ...
    recordPerformanceMetric(strategy, time) {
        if (!this.config.performance.enableMonitoring)
            return;
    ...
    startPerformanceMonitoring() {
        setInterval(() => {
            const metrics = this.getPerformanceMetrics();
            console.log(`📊 Search Performance: FTS5: ${metrics.fts5.averageTime.toFixed(1)}ms ` +
    ...
    async preWarmCache() {
        ...
        try {
            await Promise.all(commonQueries.map(query => this.search(query, { limit: 5 }).catch(() => { })));
...
exports.FTS5Integration = FTS5Integration;
exports.default = FTS5Integration;
//# sourceMappingURL=FTS5Integration.js.map