...
export class SearchBenchmark {
  ...
  async quickValidation(): Promise<{ passed: boolean; metrics: BenchmarkSummary }> {
    ...
    const passed =
      results.averageResponseTime < config.targetResponseTime &&
  ...
  async stressTest(maxConcurrency: number = 50): Promise<{
    ...
    for (let concurrency = 1; concurrency <= maxConcurrency; concurrency += 5) {
      ...
      if (breakdown.length > 1) {
        ...
        const increase =
          (result.averageResponseTime - previous.averageResponseTime) /
  ...
  private async prepareTestEnvironment(config: BenchmarkConfig): Promise<void> {
    ...
    console.log(
      `✅ Environment ready: ${this.testData.length} documents, ${this.queries.length} queries`
    );
  }
  ...
  private async simulateStressUser(
    durationSeconds: number,
    results: number[],
    errorCounter: number
  ): Promise<void> {
  ...
  private generateReport(result: BenchmarkResult): void {
    ...
    if (result.recommendations.length > 0) {
      ...
      result.recommendations.forEach((rec, index) => {
        console.log(
          `${index + 1}. [${rec.severity.toUpperCase()}] ${rec.category.toUpperCase()}: ${rec.issue}`
        );
