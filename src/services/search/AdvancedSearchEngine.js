...
class AdvancedSearchEngine {
    ...
    async initialize(entries) {
        ...
        try {
            ...
            throw new services_1.ServiceError(`Failed to initialize search engine: ${error.message}`, 'SEARCH_INIT_ERROR', 500, { originalError: error });
        }
    }
    ...
    async executeSearch(query, options, context) {
        ...
        try {
            ...
            const rankings = this.rankingEngine.rankDocuments(parsedQuery, postingLists, collection, { algorithm: this.config.rankingAlgorithm });
    ...
    async warmUpCache(entries) {
        ...
        for (const entry of entries) {
            const tokens = this.textProcessor.processText(`${entry.title} ${entry.problem} ${entry.solution}`);
