...
export class CachedSearchService {
  ...
  async invalidateCache(
    ...
    try {
      console.log(
        `🗑️ Invalidating cache: pattern=${pattern}, tags=${tags?.join(',')}, reason=${reason}`
      );
  ...
  getHealthStatus(): {
    ...
    if (
      ...
      this.config.cache.monitoring.alertThresholds.responseTime
    ) {
    ...
    // Check error rate
    const errorRate =
      this.metrics.operations.errors / Math.max(this.metrics.operations.totalQueries, 1);
  ...
  async optimizeConfiguration(): Promise<{
    ...
    const avgQueryFrequency =
      analytics.popularQueries.reduce((sum, q) => sum + q.count, 0) /
  ...
  private generateCacheKey(query: string, options: SearchOptions, context: SearchContext): string {
    const keyParts = [
      'search',
      query,
  ...
  private recordQueryMetrics(
    ...
    const totalResponseTime =
      this.metrics.performance.avgResponseTime * (this.metrics.operations.totalQueries - 1);
    this.metrics.performance.avgResponseTime =
  ...
  private updatePerformancePatterns(): void {
    ...
    for (const query of this.queryHistory) {
      ...
      const lengthBucket =
        query.query.length < 10 ? 'short' : query.query.length < 30 ? 'medium' : 'long';
      queryLength[lengthBucket] = (queryLength[lengthBucket] || 0) + 1;

      const sizeBucket =
        query.resultCount < 5 ? 'few' : query.resultCount < 20 ? 'moderate' : 'many';
  ...
  private async invalidateRelatedCache(entry: KBEntry): Promise<void> {
    ...
    const tags = [`category:${entry.category}`, ...entry.tags.map(tag => `tag:${tag}`)];
    ...
    for (const word of words.slice(0, 10)) {
      // Limit to avoid over-invalidation
