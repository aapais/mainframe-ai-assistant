...
class SearchCache {
    ...
    selectEvictionVictim(cache, strategy) {
        ...
        switch (strategy) {
            case 'lru':
                return entries.reduce((oldest, current) => current[1].lastAccessed < oldest[1].lastAccessed ? current : oldest);
            case 'lfu':
                return entries.reduce((least, current) => current[1].accessCount < least[1].accessCount ? current : least);
            case 'ttl':
                const now = Date.now();
            ...
            case 'size':
                return entries.reduce((largest, current) => current[1].size > largest[1].size ? current : largest);
            case 'adaptive':
    ...
    estimateSize(value) {
        ...
        if (typeof value === 'object' && value !== null) {
            return Object.entries(value).reduce((sum, [key, val]) => sum + key.length * 2 + this.estimateSize(val), 0) + 32;
        }
        return 8;
    }
    hashOptions(options) {
