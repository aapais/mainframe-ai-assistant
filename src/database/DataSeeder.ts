...
export class DataSeeder {
  ...
  /**
   * Seed database with comprehensive mainframe knowledge base
   */
  async seedMainframeKB(): Promise<{ seeded: number; skipped: number; errors: number }> {
    ...
    for (const entry of entries) {
      try {
        ...
        // Set initial success rate based on estimate
        if (entry.estimatedSuccessRate) {
          ...
          await this.db.recordUsage(id, true, 'system'); // Initial success
  ...
  /**
   * Get comprehensive mainframe knowledge entries
   */
  private getMainframeEntries(): MainframeKBEntry[] {
    return [
      // VSAM Errors
      {
...
6. Recent changes:
  ...
  /**
   * Find similar existing entry to avoid duplicates
   */
  private async findSimilarEntry(entry: MainframeKBEntry): Promise<boolean> {
   ...
   */
  async seedSearchHistory(): Promise<void> {
...
🚀 Knowledge Base is ready for use!`);
  ...
  async needsSeeding(): Promise<boolean> {
    ...
    return entryCount < 10; // Seed if less than 10 entries
  }
}