...
export class QueryCache {
   ...
   */
  private async analyzeFrequentQueries(): Promise<Array<{
    normalized: string;
    original: string;
    frequency: number;
    avgTime: number;
   ...
   */
  private async getCategoryUsageStats(): Promise<Array<{
    category: string;
   ...
   */
  private async getPopularSearches(): Promise<Array<{
    query: string;
    frequency: number;
  }>> {
    return this.db.prepare(`
  ...
  private async setOnDisk<T>(key: string, value: T, ttl: number): Promise<void> {
    ...
    try {
      ...
      `).run(
        key, serializedValue, 'json', Date.now(), expiresAt,
        Date.now(), dataSize, compressed
      );
    } catch (error) {
