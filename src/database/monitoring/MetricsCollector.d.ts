...
export declare class MetricsCollector extends EventEmitter {
    ...
    stopCollection(): void;
    recordMetric(metricName: string, value: number, labels?: Record<string, string>, timestamp?: number): void;
    registerMetric(metric: MetricDefinition): void;
    getMetricData(metricName: string, startTime: number, endTime: number, labels?: Record<string, string>): TimeSeriesDataPoint[];
    getAggregatedData(metricName: string, startTime: number, endTime: number, labels?: Record<string, string>): Array<AggregationResult & {
        timestamp: number;
    }>;
