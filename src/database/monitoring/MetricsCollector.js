...
class MetricsCollector extends events_1.EventEmitter {
    ...
    `);
        this.BUILTIN_METRICS.forEach(metric => {
            ...
            insertMetric.run(metric.name, metric.description, metric.unit, metric.type, JSON.stringify(metric.labels), metric.retention, metric.aggregationInterval);
        });
    }
    startCollection() {
    ...
    registerMetric(metric) {
      ...
      (name, description, unit, type, labels, retention_days, aggregation_interval)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(metric.name, metric.description, metric.unit, metric.type, JSON.stringify(metric.labels), metric.retention, metric.aggregationInterval);
    }
        ...
        const transaction = this.db.transaction(() => {
            this.dataBuffer.forEach((dataPoints, metricName) => {
                if (dataPoints.length === 0)
                    return;
                dataPoints.forEach(point => {
                    insertStmt.run(metricName, point.timestamp, point.value, point.labels ? JSON.stringify(point.labels) : null);
                });
                dataPoints.length = 0;
            });
        });
    ...
    aggregateMetricForPeriod(metricName, startTime, endTime) {
      ...
      `);
            labelGroups.forEach((values, labelsKey) => {
                ...
                const percentiles = this.calculatePercentiles(values);
                upsertStmt.run(metricName, startTime, aggregation.count, aggregation.sum, aggregation.min, aggregation.max, aggregation.avg, percentiles.p50, percentiles.p95, percentiles.p99, aggregation.stddev, labelsKey === '{}' ? null : labelsKey);
            });
        }
        catch (error) {
    ...
    recordAlert(threshold, actualValue, timestamp) {
      ...
      `).run(alertId, threshold.metricName, threshold.value, actualValue, threshold.operator, threshold.severity, threshold.description, timestamp, threshold.duration);
    ...
    getCollectionStats() {
        ...
        try {
            const dataPointsToday = this.db.prepare('SELECT COUNT(*) as count FROM time_series_data WHERE timestamp >= ?').get(todayStart)?.count || 0;
            const alertsToday = this.db.prepare('SELECT COUNT(*) as count FROM alert_history WHERE triggered_at >= ?').get(todayStart)?.count || 0;
            let bufferSize = 0;
