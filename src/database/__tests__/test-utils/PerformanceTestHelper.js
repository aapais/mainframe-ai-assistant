...
class PerformanceTestHelper extends events_1.EventEmitter {
    ...
    async runWorker(workerId, startDelay, endTime, operations, results) {
        ...
        while (Date.now() < endTime) {
            ...
            try {
                const result = await this.measureOperation(`worker-${workerId}-op-${operationCount}`, operation);
                results.push(result);
            }
            catch (error) {
    ...
    async compareImplementations(implementations, iterations = 100) {
        ...
        const fastest = Object.values(results)
            .reduce((min, current) => current.metrics.executionTime < min.metrics.executionTime ? current : min);
