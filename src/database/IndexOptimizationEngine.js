...
class IndexOptimizationEngine {
    ...
    async createOptimalIndexes(dryRun = true) {
        ...
        try {
            ...
            const estimatedImprovement = this.estimatePerformanceImprovement(createdIndexes.length, droppedIndexes.length);
    ...
    `).get();
        const queryTimeTrend = this.calculateTrend(previousAvgTime?.avg_time || 1000, recentAvgTime?.avg_time || 1000);
        return {
    ...
    determineIndexType(sql) {
        ...
        if (sql.includes('WHERE'))
            return 'PARTIAL';
        if (sql.includes('covering') || sql.split(',').length > 3)
            return 'COVERING';
    ...
    determineOptimizationType(name) {
        if (name.includes('covering'))
            return 'COVERING';
        if (name.includes('fts'))
            return 'FTS';
