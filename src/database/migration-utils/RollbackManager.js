...
class RollbackManager extends events_1.EventEmitter {
    ...
    async testRollback(fromVersion, toVersion) {
        const tempDbPath = path_1.default.join(path_1.default.dirname(this.db.name), `test_rollback_${Date.now()}.db`);
    ...
    async saveRollbackPoint(rollbackPoint) {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(rollbackPoint.id, rollbackPoint.version, rollbackPoint.backupPath, rollbackPoint.description, rollbackPoint.rollbackSql, rollbackPoint.checksum);
    }
    ...
    async createDatabaseBackup(version) {
        ...
        const backupPath = path_1.default.join(path_1.default.dirname(this.db.name), `rollback_backup_v${version}_${timestamp}.db`);
