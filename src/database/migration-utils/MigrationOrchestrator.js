...
class MigrationOrchestrator extends events_1.EventEmitter {
    ...
    async analyzeUpgradePath() {
        ...
        if (pendingMigrations.length > 0) {
            ...
            const highRiskMigrations = pendingMigrations.filter(m => m.description.toLowerCase().includes('breaking') ||
    ...
    assessRiskLevel(migrations) {
        ...
        for (const migration of migrations) {
            ...
            if (migration.up.includes('DROP COLUMN'))
                riskScore += 8;
            if (migration.up.includes('ALTER TABLE') && migration.up.includes('DROP'))
                riskScore += 6;
            if (migration.up.includes('DELETE FROM'))
                riskScore += 5;
            if (migration.up.includes('UPDATE') && !migration.up.includes('WHERE'))
                riskScore += 7;
            if (migration.description.toLowerCase().includes('breaking'))
                riskScore += 8;
            if (migration.description.toLowerCase().includes('major'))
                riskScore += 6;
        }
        if (riskScore >= 20)
            return 'critical';
    ...
    assessDowntimeRequirement(migrations) {
        for (const migration of migrations) {
            if (migration.up.includes('DROP TABLE'))
                return true;
            if (migration.up.includes('ALTER TABLE') && migration.up.includes('DROP COLUMN'))
                return true;
            if (migration.up.includes('PRAGMA foreign_keys = OFF'))
                return true;
            if (migration.description.toLowerCase().includes('breaking'))
                return true;
        }
    ...
    async createPreMigrationBackup(plan) {
        ...
        const backupPath = path_1.default.join(path_1.default.dirname(this.db.name), `backup_pre_migration_v${plan.currentVersion}_to_v${plan.targetVersion}_${timestamp}.db`);
