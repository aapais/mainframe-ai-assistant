...
class DatabaseManager extends events_1.EventEmitter {
  ...
  constructor(config) {
    ...
    this.config = {
      ...
      backup: {
        ...
        path:
          config.backup?.path ??
          path_1.default.join(path_1.default.dirname(config.path), 'backups'),
      },
      queryCache: {
  ...
  async initialize() {
    ...
    try {
      ...
      this.migrationManager = new MigrationManager_1.MigrationManager(
        this.db,
        path_1.default.join(path_1.default.dirname(this.config.path), 'migrations')
      );
