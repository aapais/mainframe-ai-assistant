...
class KnowledgeBaseModel {
    ...
    initializePreparedStatements() {
      ...
      `);
            ...
            throw new AppError_1.AppError(AppError_1.ErrorCode.DATABASE_INITIALIZATION_ERROR, 'Failed to initialize prepared statements', { originalError: error });
        }
    }
    async createEntry(entryData, userId) {
        return this.executeWithPerformanceTracking('createEntry', async () => {
            const validationResult = KnowledgeBase_schema_1.SchemaValidator.safeParse(KnowledgeBase_schema_1.DatabaseSchemas.CreateKBEntry, entryData);
            if (!validationResult.success) {
                throw new AppError_1.AppError(AppError_1.ErrorCode.VALIDATION_ERROR, 'Invalid KB entry data', { validationErrors: validationResult.error });
            }
            const validatedData = validationResult.data;
            const id = (0, uuid_1.v4)();
            const transaction = this.db.transaction(() => {
                try {
                    this.statements.insertEntry.run(id, validatedData.title, validatedData.problem, validatedData.solution, validatedData.category, validatedData.severity || 'medium', userId || 'system');
                    ...
                    throw new AppError_1.AppError(AppError_1.ErrorCode.DATABASE_INSERT_ERROR, 'Failed to create KB entry', { entryId: id, originalError: error });
                }
            });
    ...
    async updateEntry(id, updates, userId) {
        return this.executeWithPerformanceTracking('updateEntry', async () => {
            const validationResult = KnowledgeBase_schema_1.SchemaValidator.safeParse(KnowledgeBase_schema_1.DatabaseSchemas.UpdateKBEntry, updates);
            if (!validationResult.success) {
                throw new AppError_1.AppError(AppError_1.ErrorCode.VALIDATION_ERROR, 'Invalid update data', { validationErrors: validationResult.error });
            }
            ...
            const transaction = this.db.transaction(() => {
                try {
                    if (validatedUpdates.title !== undefined ||
                        ...
                        this.statements.updateEntry.run(validatedUpdates.title ?? existingEntry.data.title, validatedUpdates.problem ?? existingEntry.data.problem, validatedUpdates.solution ?? existingEntry.data.solution, validatedUpdates.category ?? existingEntry.data.category, validatedUpdates.severity ?? existingEntry.data.severity, validatedUpdates.archived ?? existingEntry.data.archived, id);
                    }
                    ...
                    throw new AppError_1.AppError(AppError_1.ErrorCode.DATABASE_UPDATE_ERROR, 'Failed to update KB entry', { entryId: id, originalError: error });
                }
            });
            transaction();
            this.logUsageMetric({
    ...
    async getEntryById(id) {
        return this.executeWithPerformanceTracking('getEntryById', async () => {
            if (!id || typeof id !== 'string') {
                throw new AppError_1.AppError(AppError_1.ErrorCode.VALIDATION_ERROR, 'Invalid entry ID', { entryId: id });
            }
            try {
                ...
                const validationResult = KnowledgeBase_schema_1.SchemaValidator.safeParse(KnowledgeBase_schema_1.DatabaseSchemas.KBEntry, entry);
                if (!validationResult.success) {
                    throw new AppError_1.AppError(AppError_1.ErrorCode.DATA_CONSISTENCY_ERROR, 'Entry data validation failed after retrieval', { entryId: id, validationErrors: validationResult.error });
                }
                this.logUsageMetric({
                    entry_id: id,
                    action: 'view',
                ...
                throw new AppError_1.AppError(AppError_1.ErrorCode.DATABASE_QUERY_ERROR, 'Failed to retrieve KB entry', { entryId: id, originalError: error });
            }
        });
    }
    async deleteEntry(id, userId) {
        return this.executeWithPerformanceTracking('deleteEntry', async () => {
          ...
          `).run(id);
                    ...
                    throw new AppError_1.AppError(AppError_1.ErrorCode.DATABASE_DELETE_ERROR, 'Failed to delete KB entry', { entryId: id, originalError: error });
                }
            });
            transaction();
            this.logUsageMetric({
    ...
    async search(searchQuery) {
        return this.executeWithPerformanceTracking('search', async () => {
            const validationResult = KnowledgeBase_schema_1.SchemaValidator.safeParse(KnowledgeBase_schema_1.DatabaseSchemas.SearchQuery, searchQuery);
            if (!validationResult.success) {
                throw new AppError_1.AppError(AppError_1.ErrorCode.VALIDATION_ERROR, 'Invalid search query', { validationErrors: validationResult.error });
            }
            ...
            try {
                ...
                throw new AppError_1.AppError(AppError_1.ErrorCode.SEARCH_ERROR, 'Search operation failed', { query: validatedQuery.query, originalError: error });
            }
        });
    }
    ...
    async recordFeedback(feedback) {
        return this.executeWithPerformanceTracking('recordFeedback', async () => {
            const validationResult = KnowledgeBase_schema_1.SchemaValidator.safeParse(KnowledgeBase_schema_1.DatabaseSchemas.EntryFeedback, feedback);
            if (!validationResult.success) {
                throw new AppError_1.AppError(AppError_1.ErrorCode.VALIDATION_ERROR, 'Invalid feedback data', { validationErrors: validationResult.error });
            }
            const validatedFeedback = validationResult.data;
            const feedbackId = (0, uuid_1.v4)();
            try {
                this.statements.insertFeedback.run(feedbackId, validatedFeedback.entry_id, validatedFeedback.user_id, validatedFeedback.rating, validatedFeedback.successful, validatedFeedback.comment, validatedFeedback.session_id);
                this.updateEntryMetrics(validatedFeedback.entry_id, validatedFeedback.successful);
                return feedbackId;
            }
            catch (error) {
                throw new AppError_1.AppError(AppError_1.ErrorCode.DATABASE_INSERT_ERROR, 'Failed to record feedback', { originalError: error });
            }
        });
    }
    async logUsageMetric(metric) {
        try {
            if (!this.statements.insertUsageMetric)
                return;
            this.statements.insertUsageMetric.run(metric.entry_id, metric.action, metric.user_id || 'anonymous', metric.session_id, metric.metadata ? JSON.stringify(metric.metadata) : null);
        }
        catch (error) {
    ...
    async logSearchHistory(history) {
        try {
            if (!this.statements.insertSearchHistory)
                return;
            this.statements.insertSearchHistory.run(history.query, history.normalized_query, history.results_count, history.selected_entry_id, history.user_id || 'anonymous', history.session_id, history.search_time_ms, history.filters_used ? JSON.stringify(history.filters_used) : null, history.ai_used);
        }
        catch (error) {
    ...
    async getStatistics() {
        ...
        `).all();
                ...
                const validationResult = KnowledgeBase_schema_1.SchemaValidator.safeParse(KnowledgeBase_schema_1.DatabaseSchemas.DatabaseStats, stats);
                if (!validationResult.success) {
                    throw new AppError_1.AppError(AppError_1.ErrorCode.DATA_CONSISTENCY_ERROR, 'Statistics validation failed', { validationErrors: validationResult.error });
                }
                return validationResult.data;
            }
            catch (error) {
                ...
                throw new AppError_1.AppError(AppError_1.ErrorCode.DATABASE_QUERY_ERROR, 'Failed to retrieve statistics', { originalError: error });
            }
        });
    }
    async executeWithPerformanceTracking(operation, callback) {
        ...
        try {
            ...
            return {
                success: false,
                error: new AppError_1.AppError(AppError_1.ErrorCode.UNKNOWN_ERROR, `Operation ${operation} failed`, { originalError: error }),
                performance: {
                    executionTime,
                    queriesExecuted,
                    cacheHit
    ...
    transformRowToKBEntry(row) {
        ...
        const validationResult = KnowledgeBase_schema_1.SchemaValidator.safeParse(KnowledgeBase_schema_1.DatabaseSchemas.KBEntry, entry);
        if (!validationResult.success) {
            throw new AppError_1.AppError(AppError_1.ErrorCode.DATA_CONSISTENCY_ERROR, 'Invalid entry data from database', { entryId: row.id, validationErrors: validationResult.error });
        }
        return entry;
    }
    ...
    determineMatchType(query, row) {
        if (query.query === row.title)
            return 'exact';
        if (query.category && query.category === row.category)
            return 'category';
        if (query.useAI)
            return 'ai';
        return 'fuzzy';
