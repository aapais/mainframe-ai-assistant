...
import { KBEntry, CreateKBEntry, UpdateKBEntry, SearchQuery, SearchResult, SearchWithFacets, EntryFeedback, UsageMetric, SearchHistory, DatabaseStats, KBCategory } from '../schemas/KnowledgeBase.schema';
...
export interface IKnowledgeBaseRepository extends IRepository<KBEntry, CreateKBEntry, UpdateKBEntry> {
    ...
    getAllCategories(): Promise<RepositoryResult<Array<{
        name: string;
        count: number;
    }>>>;
    getAllTags(): Promise<RepositoryResult<Array<{
        name: string;
        count: number;
    }>>>;
