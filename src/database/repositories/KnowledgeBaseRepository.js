...
class KnowledgeBaseRepository {
  ...
  async delete(id) {
    return this.executeWithMetrics('delete', async () => {
      ...
      if (result.changes === 0) {
        throw new AppError_1.AppError(AppError_1.ErrorCode.RESOURCE_NOT_FOUND, 'Entry not found', {
          entryId: id,
        });
      }
      await this.recordUsage({
  ...
  async search(query) {
    return this.executeWithMetrics('search', async () => {
      ...
      try {
        ...
        switch (strategy) {
          case 'category':
            const categoryResults = await this.findByCategory(validatedQuery.category, {
  ...
  async findByTags(tags, options = {}) {
    return this.executeWithMetrics('findByTags', async () => {
      ...
      const rows = this.statements
        .get('findByTags')
        .all(...paddedTags, ...paddedTags, limit, offset);
      return rows.map(row => ({
        entry: this.transformRowToKBEntry(row),
      ...
      return {
        ...
        error:
          error instanceof AppError_1.AppError
            ? error
            : AppError_1.AppError.fromUnknown(error, { operation }),
        metadata: {
          executionTime,
          cacheHit: false,
        },
