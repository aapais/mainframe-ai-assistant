...
class TagRepository {
    ...
    async create(tagData, userId) {
        const validatedData = HierarchicalCategories_schema_1.HierarchicalSchemaValidator.validateCreateTag(tagData);
        const transaction = this.db.transaction(() => {
            const existing = this.preparedStatements.get('selectByName').get(validatedData.name);
            if (existing) {
                throw new AppError_1.AppError(AppError_1.ErrorCode.VALIDATION_ERROR, 'Tag name already exists');
            }
            if (validatedData.category_id) {
                const category = this.db.prepare('SELECT id FROM categories WHERE id = ?').get(validatedData.category_id);
                if (!category) {
                    throw new AppError_1.AppError(AppError_1.ErrorCode.VALIDATION_ERROR, 'Category not found');
                }
            }
            const id = (0, uuid_1.v4)();
            this.preparedStatements.get('insertTag').run(id, validatedData.name, validatedData.display_name, validatedData.description || null, validatedData.category_id || null, validatedData.color || null, validatedData.is_system === true, validatedData.is_suggested === true, userId || 'system');
    ...
    async update(id, updates, userId) {
        ...
        const transaction = this.db.transaction(() => {
            ...
            if (existing.is_system) {
                throw new AppError_1.AppError(AppError_1.ErrorCode.VALIDATION_ERROR, 'Cannot modify system tag');
            }
            ...
            this.preparedStatements.get('updateTag').run(validatedUpdates.name, validatedUpdates.display_name, validatedUpdates.description, validatedUpdates.category_id, validatedUpdates.color, validatedUpdates.is_suggested, id);
            return this.preparedStatements.get('selectById').get(id);
        });
        return transaction();
    }
    async delete(id, options) {
        const transaction = this.db.transaction(() => {
            ...
            if (tag.is_system && !options?.force) {
                throw new AppError_1.AppError(AppError_1.ErrorCode.VALIDATION_ERROR, 'Cannot delete system tag');
            }
            const associationCount = this.db.prepare('SELECT COUNT(*) as count FROM tag_associations WHERE tag_id = ?').get(id);
            if (associationCount.count > 0 && !options?.force) {
                throw new AppError_1.AppError(AppError_1.ErrorCode.VALIDATION_ERROR, `Tag is associated with ${associationCount.count} entries. Use force option to delete.`);
            }
            ...
            if (result.changes === 0) {
                throw new AppError_1.AppError(AppError_1.ErrorCode.RESOURCE_NOT_FOUND, 'Tag not found or is system tag');
            }
        });
        transaction();
    }
    ...
    async associateWithEntry(entryId, tagId, options = {}, userId) {
        const transaction = this.db.transaction(() => {
            ...
            if (!entry) {
                throw new AppError_1.AppError(AppError_1.ErrorCode.RESOURCE_NOT_FOUND, 'KB entry not found');
            }
            const tag = this.preparedStatements.get('selectById').get(tagId);
            if (!tag) {
                throw new AppError_1.AppError(AppError_1.ErrorCode.RESOURCE_NOT_FOUND, 'Tag not found');
            }
            const existing = this.db.prepare('SELECT id FROM tag_associations WHERE entry_id = ? AND tag_id = ?').get(entryId, tagId);
            if (existing) {
                throw new AppError_1.AppError(AppError_1.ErrorCode.VALIDATION_ERROR, 'Tag is already associated with this entry');
            }
            const associationId = (0, uuid_1.v4)();
            this.preparedStatements.get('insertAssociation').run(associationId, entryId, tagId, options.relevanceScore || null, options.assignedBy || 'user', options.confidence || null, userId || 'system');
        });
        transaction();
    }
    async dissociateFromEntry(entryId, tagId) {
        ...
        if (result.changes === 0) {
            throw new AppError_1.AppError(AppError_1.ErrorCode.RESOURCE_NOT_FOUND, 'Tag association not found');
        }
    }
    ...
    async replaceEntryTags(entryId, tagIds, options = {}, userId) {
        const transaction = this.db.transaction(() => {
            ...
            for (const tagId of tagIds) {
                const tag = this.preparedStatements.get('selectById').get(tagId);
                if (!tag) {
                    throw new AppError_1.AppError(AppError_1.ErrorCode.RESOURCE_NOT_FOUND, `Tag ${tagId} not found`);
                }
                const associationId = (0, uuid_1.v4)();
                this.preparedStatements.get('insertAssociation').run(associationId, entryId, tagId, options.relevanceScore || null, options.assignedBy || 'user', options.confidence || null, userId || 'system');
            }
    ...
    async updateAnalytics(tagId, analytics) {
        this.preparedStatements.get('updateTagAnalytics').run(tagId, analytics.usage_count || 0, analytics.entry_count || 0);
    }
