...
class QueryCache {
  ...
  calculateTTLBasedOnUsage(frequency) {
    if (frequency > 100) return 1800000;
    if (frequency > 50) return 900000;
    if (frequency > 20) return 600000;
    if (frequency > 10) return 300000;
    return 180000;
  }
  ...
  enforceMemoryLimit() {
    ...
    const targetEviction = Math.max(
      Math.ceil(this.memoryCache.size * 0.1),
      this.memoryCache.size - this.config.maxSize + 100
    );
