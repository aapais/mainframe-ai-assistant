...
const SettingsModal = ({ isOpen, onClose, user }) => {
    ...
    const resetSettings = async () => {
        ...
        setSaving(true);
        try {
            const response = await fetch(`/api/settings/${user.id}/reset`, {
                method: 'POST'
            });
    ...
    const exportSettings = async () => {
        try {
            const response = await fetch(`/api/settings/${user.id}/export`);
    ...
    const importSettings = (event) => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                const response = await fetch(`/api/settings/${user.id}/import`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    return React.createElement('div', {
        ...
        React.createElement('div', {
            ...
            React.createElement('div', {
                key: 'header',
                className: 'flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-600'
            }, [
            ...
            React.createElement('div', {
                ...
                React.createElement('div', {
                    key: 'sidebar',
                    className: 'w-64 bg-gray-50 dark:bg-gray-700 border-r border-gray-200 dark:border-gray-600 overflow-y-auto'
                }, [
                ...
                React.createElement('div', {
                    ...
                    settings ? 
                        React.createElement('div', {
                            ...
                            activeTab === 'theme' && React.createElement(window.ThemeSettings || 'div', {
                                key: 'theme-settings',
                                settings,
                                onUpdate: updateSettings,
                                onMarkChanged: () => setHasChanges(true)
                            }),
                            activeTab === 'language' && React.createElement(window.LanguageSettings || 'div', {
                                key: 'language-settings',
                                settings,
                                onUpdate: updateSettings,
                                onMarkChanged: () => setHasChanges(true)
                            }),
                            activeTab === 'notifications' && React.createElement(window.NotificationSettings || 'div', {
                                key: 'notification-settings',
                                settings,
                                onUpdate: updateSettings,
                                onMarkChanged: () => setHasChanges(true)
                            }),
                            activeTab === 'display' && React.createElement(window.DisplaySettings || 'div', {
                                key: 'display-settings',
                                settings,
                                onUpdate: updateSettings,
                                onMarkChanged: () => setHasChanges(true)
                            }),
                            activeTab === 'apikeys' && React.createElement(window.ApiKeySettings || 'div', {
                                key: 'apikey-settings',
                                settings,
                                onUpdate: updateSettings,
                                onMarkChanged: () => setHasChanges(true)
                            }),
                            activeTab === 'data' && React.createElement(window.DataSettings || 'div', {
                                key: 'data-settings',
                                settings,
            ...
            React.createElement('div', {
                key: 'footer',
                className: 'flex items-center justify-between p-6 border-t border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700'
            }, [
                React.createElement('div', {
                    key: 'status',
