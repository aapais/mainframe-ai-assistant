...
const DataSettings = ({ settings, onExport, onImport, onReset }) => {
    ...
    const [searchHistoryEnabled, setSearchHistoryEnabled] = useState(settings?.search_history_enabled !== false);
    const [recentSearchesLimit, setRecentSearchesLimit] = useState(settings?.recent_searches_limit || 10);
    ...
    const handleReset = async () => {
        if (confirm('This will reset ALL settings to default values and cannot be undone. Are you sure?')) {
            await onReset();
        }
    };
    ...
    const clearAllLocalData = () => {
        if (confirm('This will delete ALL local data including settings, cache, and search history. This action cannot be undone. Are you sure?')) {
    ...
    return React.createElement('div', {
                ...
                React.createElement('div', {
                    ...
                    React.createElement('div', {
                        ...
                        className: `text-lg font-semibold ${
                            autoSave ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                        }`
                    }, autoSave ? 'On' : 'Off'),
                    React.createElement('div', {
                        key: 'autosave-label',
        ...
        React.createElement('div', {
            ...
            React.createElement('div', {
                key: 'import-warning',
                className: 'bg-yellow-50 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-700 rounded p-3'
            }, [
                React.createElement('p', {
                    key: 'warning-text',
        ...
        React.createElement('div', {
            ...
            autoSave && React.createElement('div', {
        ...
        React.createElement('div', {
            ...
            React.createElement('div', {
                ...
                React.createElement('button', {
                    ...
                    className: 'block w-full px-4 py-2 bg-yellow-600 text-white rounded hover:bg-yellow-700 text-left'
                }, '🔍 Clear Search History'),
                
                React.createElement('button', {
                    key: 'reset-settings',
                    onClick: handleReset,
                    className: 'block w-full px-4 py-2 bg-orange-600 text-white rounded hover:bg-orange-700 text-left'
