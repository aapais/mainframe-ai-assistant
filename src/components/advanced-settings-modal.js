...
window.AdvancedSettingsModal = function ({ isOpen, onClose, userId }) {
  ...
  const loadSettings = async () => {
    setLoading(true);
    try {
      // Load settings directly from backend instead of SettingsService
      if (userId) {
        const response = await fetch(`http://localhost:3001/api/settings/${userId}`);
        const result = await response.json();
        if (result.success && result.settings) {
          // Parse settings_json if exists
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setHasChanges(false);
        // Also update cache in SettingsService
        if (window.SettingsService) {
          window.SettingsService.cache.set(userId, settings);
        }
      ...
      React.createElement('div', { key: 'api-keys', className: 'space-y-4' }, [
        ...
        React.createElement('div', { key: 'openai' }, [
          ...
          React.createElement('input', {
            ...
            onChange: e =>
              updateSetting('api_keys', { ...settings.api_keys, openai: e.target.value }),
        ...
        React.createElement('div', { key: 'anthropic' }, [
          ...
          React.createElement('input', {
            ...
            onChange: e =>
              updateSetting('api_keys', { ...settings.api_keys, anthropic: e.target.value }),
            className: 'w-full px-3 py-2 border rounded-lg font-mono text-sm',
            placeholder: 'sk-ant-...',
          }),
        ]),
        ...
        React.createElement('div', { key: 'google' }, [
          ...
          React.createElement('input', {
            ...
            onChange: e =>
              updateSetting('api_keys', { ...settings.api_keys, google: e.target.value }),
            className: 'w-full px-3 py-2 border rounded-lg font-mono text-sm',
            placeholder: 'AIza...',
          }),
        ]),
    ...
    return React.createElement('div', { className: 'space-y-6' }, [
      ...
      React.createElement('div', { key: 'display-settings', className: 'space-y-4' }, [
        ...
        React.createElement('div', { key: 'toggles', className: 'space-y-3' }, [
          ...
          React.createElement(
            'label',
            { key: 'line-numbers', className: 'flex items-center gap-3' },
            [
              React.createElement('input', {
                type: 'checkbox',
          ...
          React.createElement(
            'label',
            { key: 'animations', className: 'flex items-center gap-3' },
            [
              React.createElement('input', {
                type: 'checkbox',
  ...
  const renderDataTab = () => {
    ...
    return React.createElement('div', { className: 'space-y-6' }, [
      ...
      React.createElement('div', { key: 'autosave', className: 'space-y-4 pt-4 border-t' }, [
        ...
        React.createElement(
          'label',
          { key: 'autosave-toggle', className: 'flex items-center gap-3' },
          [
            React.createElement('input', {
              type: 'checkbox',
  ...
  const renderSystemTab = () => {
    ...
    return React.createElement('div', { className: 'space-y-6' }, [
      ...
      React.createElement('div', { key: 'info', className: 'space-y-2 pt-4 border-t' }, [
        ...
        React.createElement(
          'div',
          { className: 'bg-gray-50 p-3 rounded-lg space-y-1 text-sm font-mono' },
          [
    ...
    [
      React.createElement(
        'div',
        {
          key: 'modal-content',
          className:
            'bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden fade-in',
        },
        [
          // Header
          React.createElement(
            'div',
            {
              key: 'header',
              className:
                'px-6 py-4 border-b flex items-center justify-between bg-gradient-to-r from-gray-800 to-gray-700',
            },
            [
              React.createElement(
                'h2',
          ...
          React.createElement(
            ...
            [
              React.createElement(
                'div',
                { key: 'warning', className: 'text-xs text-amber-600 flex items-center gap-2' },
                [
                  React.createElement('span', {}, '⚠️'),
                  React.createElement(
                    'span',
                    {},
                    'Estas são configurações avançadas. Alterações incorretas podem afetar o sistema.'
                  ),
                ]
              ),
