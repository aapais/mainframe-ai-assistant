...
export const SecurityDashboard: React.FC<SecurityDashboardProps> = ({ className }) => {
  ...
  const loadSecurityData = async () => {
    try {
      setIsLoading(true);
      const [events, score] = await Promise.all([getSecurityEvents(), calculateSecurityScore()]);
    ...
    const factors = {
      ...
      accountAge: {
        score: Math.min(
          100,
          Math.floor(
            ((Date.now() - new Date(user.createdAt).getTime()) / (1000 * 60 * 60 * 24 * 365)) * 20
          )
        ), // 20 points per year, max 100
        weight: 0.1,
      },
      emailVerified: {
      ...
      {criticalEvents.length > 0 && (
        <Alert variant='destructive'>
          ...
          <AlertDescription>
            <div className='flex items-center justify-between'>
              <span>
                You have {criticalEvents.length} critical security alert
                {criticalEvents.length !== 1 ? 's' : ''} requiring attention
              </span>
