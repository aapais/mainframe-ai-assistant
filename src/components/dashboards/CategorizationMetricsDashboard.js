...
const OverviewTab = ({ metrics, routingMetrics }) => {
    ...
    const routingSuccessRate = (routingMetrics.successfulRoutings / routingMetrics.totalRoutings) * 100;
                    ...
                    <CardContent>
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12} md={4}>
                                ...
                                >
                                    <Typography variant="subtitle2">
                                        SLA: {routingMetrics.slaBreachRate <= 0.1 ? 'Dentro do esperado' : 'Violações detectadas'}
                                    </Typography>
                                    <Typography variant="body2">
                            ...
                            <Grid item xs={12} md={4}>
                                <Alert
                                    severity={metrics.cacheHitRate >= 0.7 ? 'success' : 'info'}
                                    icon={<Speed />}
                                >
                                    <Typography variant="subtitle2">
                                ...
                                <TableBody>
                                    {Object.entries(metrics.methodPerformance).map(([method, data]) => (
                                        <TableRow key={method}>
                                            ...
                                            <TableCell align="right">
                                                <Chip
                                                    label={data.accuracy >= 0.8 ? 'Ótimo' : data.accuracy >= 0.6 ? 'Bom' : 'Baixo'}
                                                    size="small"
                                                    color={data.accuracy >= 0.8 ? 'success' : data.accuracy >= 0.6 ? 'warning' : 'error'}
                                                />
                                            </TableCell>
                                        </TableRow>
                                    ))}
                    ...
                    <CardContent>
                        ...
                        <TableContainer>
                            <Table>
                                ...
                                <TableBody>
                                    {Object.entries(routingMetrics.teamUtilizationRates).map(([team, utilization]) => (
                                        <TableRow key={team}>
                                            <TableCell component="th" scope="row">
    ...
    return {
        ...
        datasets: [{
            ...
            backgroundColor: Object.values(utilization).map(value =>
                value > 0.8 ? 'rgba(255, 99, 132, 0.8)' :
                value > 0.6 ? 'rgba(255, 206, 86, 0.8)' :
                'rgba(75, 192, 192, 0.8)'
            )
        }]
    };
