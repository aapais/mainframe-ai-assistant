...
export class VersionControlService extends EventEmitter {
  ...
  async recordChange(
    ...
    const fieldChanges: FieldChange[] = changes ? Object.entries(changes).map(([field, change]) => ({
      field,
   ...
   */
  async getEntryHistory(entryId: string, options: Partial<HistoryQuery> = {}): Promise<EntryVersion[]> {
    // Check cache first
  ...
  async getChangeDetails(changeId: string): Promise<ChangeRecord | null> {
    ...
    return {
      ...
      fieldChanges,
      beforeSnapshot: versions.length > 1 ? this.decompressSnapshot(versions[0].snapshot_data) : undefined,
      afterSnapshot: versions.length > 0 ? this.decompressSnapshot(versions[versions.length - 1].snapshot_data) : undefined,
   ...
   */
  async compareVersions(entryId: string, fromVersion: number, toVersion: number): Promise<VersionDiff> {
    const versions = await this.getSpecificVersions(entryId, [fromVersion, toVersion]);

    if (versions.length !== 2) {
      throw new Error(`Could not find both versions ${fromVersion} and ${toVersion} for entry ${entryId}`);
    }
    ...
    const summary = {
      ...
      significantChange: fieldChanges.some(c =>
        this.SIGNIFICANT_FIELDS.includes(c.field) && c.changeType !== 'removed'
      )
    };
  ...
  async rollbackToVersion(
    ...
    try {
      ...
      const targetVersionRecord = this.db.prepare(`
        SELECT * FROM entry_versions
  ...
  async detectMergeConflicts(
    ...
    Object.entries(incomingChanges).forEach(([field, incomingValue]) => {
      ...
      } else if (baseValue !== undefined && currentValue === undefined && incomingValue !== undefined) {
      ...
      } else if (baseValue !== undefined && incomingValue === undefined && currentValue !== baseValue) {
  ...
  async getVersionStats(entryId: string): Promise<{
    ...
    const changeTypes = this.db.prepare(`
  ...
  async cleanupOldVersions(dryRun: boolean = false): Promise<{
    ...
    const entriesToProcess = this.db.prepare(`
    ...
    for (const entry of entriesToProcess) {
      try {
        const versionsToRemove = this.db.prepare(`
        ...
        `).all(entry.entry_id, cutoffDate.toISOString(), entry.entry_id, Math.min(this.MAX_VERSIONS_PER_ENTRY, 10));
  ...
  private calculateFieldChanges(fromSnapshot: Partial<KBEntry>, toSnapshot: Partial<KBEntry>): FieldChange[] {
  ...
  private filterVersionsByOptions(versions: EntryVersion[], options: Partial<HistoryQuery>): EntryVersion[] {
    ...
    if (options.dateRange) {
      filtered = filtered.filter(v =>
        v.timestamp >= options.dateRange!.start && v.timestamp <= options.dateRange!.end
      );
    }
  ...
  private invalidateEntryCache(entryId: string): void {
    const keysToRemove = Array.from(this.versionCache.keys())
      .filter(key => key.includes(entryId));
  ...
  private cleanupEntryVersions(entryId: string): void {
    ...
    const versionsToDelete = this.db.prepare(`
