...
describe('ConnectionPool', () => {
  ...
  test('should execute queries', async () => {
    ...
    // Insert test data
    const insertResult = await connectionPool.executeQuery('INSERT INTO test (name) VALUES (?)', [
      'Test Name',
    ]);
    expect(insertResult).toBeDefined();

    // Query test data
    const selectResult = await connectionPool.executeQuery('SELECT * FROM test WHERE name = ?', [
      'Test Name',
    ]);
