...
export class KnowledgeBaseHandler {
  ...
  handleLocalSearch: IPCHandlerFunction<'kb:search:local'> = async (request) => {
    ...
    try {
      ...
      await this.recordSearchMetrics(query, searchResults.length, 'local', queryResult.executionTime);
  ...
  private validateEntryUpdates(updates: any): { valid: boolean; error?: string } {
    ...
    if (updates.problem !== undefined && (typeof updates.problem !== 'string' || updates.problem.trim().length === 0)) {
      return { valid: false, error: 'Problem must be a non-empty string' };
    }
    
    if (updates.solution !== undefined && (typeof updates.solution !== 'string' || updates.solution.trim().length === 0)) {
  ...
  private async findSimilarEntries(entry: KBEntryInput): Promise<KBEntry[]> {
    ...
    const result = await this.dbManager.query<any[]>(sql, [entry.category, titlePattern, problemPattern]);
  ...
  private async recordSearchMetrics(query: string, resultCount: number, searchType: string, executionTime: number): Promise<void> {
    try {
      await this.dbManager.query(
