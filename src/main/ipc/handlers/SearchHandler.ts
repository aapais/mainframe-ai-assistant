...
import { KBEntry, SearchResult, KBCategory } from '../../../types/index';
...
export class SearchHandler {
  ...
  private searchCache = new Map<
    string,
    { results: SearchResult[]; timestamp: number; ttl: number }
  >();
  ...
  handleAISearch: IPCHandlerFunction<'kb:search:ai'> = async request => {
    ...
    try {
      ...
      if (cached) {
        ...
        return this.createSuccessResponse(request.requestId, startTime, cached, {
          cached: true,
          searchType: 'semantic',
          fromCache: 'memory',
          aiConfidence: 90,
      ...
      return this.createSuccessResponse(request.requestId, startTime, searchResults, {
        cached: false,
        searchType: 'semantic',
  ...
  private async performAIAnalysis(query: string, options: any): Promise<AISearchContext> {
    ...
    try {
      ...
      if (cachedExpansion && Date.now() - cachedExpansion.timestamp < 24 * 60 * 60 * 1000) {
        // 24 hours
        return {
          originalQuery: query,
  ...
  private buildOptimizedSearchQuery(query: string, options: any): { sql: string; params: any[] } {
    const { categories, tags, includeArchived, sortBy, sortOrder, limit, offset, minConfidence } =
      options;
  ...
  private calculateAdvancedSearchScores(
    entries: KBEntry[],
    query: string,
    options: any
  ): SearchResult[] {
    ...
    return entries
      .map(entry => {
        ...
        entry.tags.forEach(tag => {
          ...
          if (tagLower.includes(queryLower) || queryWords.some(word => tagLower.includes(word))) {
            score += 12;
            tagMatches++;
            highlights.push(tag);
          }
...
Format your response as JSON:
{
  ...
  private async callGeminiAPI(prompt: string): Promise<any> {
    ...
    throw new Error(
      `Gemini API failed after ${this.geminiConfig.maxRetries} attempts: ${lastError.message}`
    );
  }
  ...
  private updatePerformanceMetrics(latency: number, success: boolean, cached: boolean): void {
    ...
    const totalLatency =
      this.performanceMetrics.averageLatency * (this.performanceMetrics.totalSearches - 1);
    this.performanceMetrics.averageLatency =
      (totalLatency + latency) / this.performanceMetrics.totalSearches;

    // Update success rates
    if (success) {
      this.performanceMetrics.aiSuccessRate =
        (this.performanceMetrics.aiSuccessRate * (this.performanceMetrics.totalSearches - 1) +
          100) /
        this.performanceMetrics.totalSearches;
    } else {
      this.performanceMetrics.errorRate =
        (this.performanceMetrics.errorRate * (this.performanceMetrics.totalSearches - 1) + 100) /
        this.performanceMetrics.totalSearches;
    }
    ...
    if (cached) {
      this.performanceMetrics.cacheHitRate =
        (this.performanceMetrics.cacheHitRate * (this.performanceMetrics.totalSearches - 1) + 100) /
        this.performanceMetrics.totalSearches;
    }
