...
import { IPCChannel, IPCErrorCode, IPCError, IPCSchemas, BaseIPCRequest } from '../../../types/ipc';
...
class InputSanitizer {
  ...
  private readonly allowedTags = new Set(['b', 'i', 'u', 'strong', 'em', 'code', 'pre', 'br', 'p']);
...
export class IPCSecurityManager {
  ...
  getSecurityMetrics(): {
    ...
    const eventsBySeverity = this.securityEvents.reduce(
      (acc, event) => {
    ...
    setInterval(
      () => {
        ...
        if (originalLength !== this.securityEvents.length) {
          console.debug(
            `Cleaned up ${originalLength - this.securityEvents.length} old security events`
          );
        }
      },
      60 * 60 * 1000
