...
function ConnectionStatus() {
  const { isConnected, isOnline, connectionError, aiServiceAvailable, databaseConnected } =
    useIPC();
...
function KnowledgeBaseSection({
  ...
  return (
    <div className='knowledge-base-section'>
      ...
      {kb.isOptimistic && (
        <div className='optimistic-indicator'>🔄 Optimistic update in progress...</div>
      )}
        ...
        {kb.entries.slice(0, 5).map(result => (
          <div key={result.entry.id} className='kb-entry'>
            ...
            <div className='entry-actions'>
              <button onClick={() => onRateEntry(result.entry.id, true)} disabled={kb.isLoading}>
                👍 Helpful
              </button>
              <button onClick={() => onRateEntry(result.entry.id, false)} disabled={kb.isLoading}>
                👎 Not Helpful
              </button>
              <button onClick={() => kb.selectEntry(result.entry.id)} disabled={kb.isLoading}>
                View Details
              </button>
            </div>
          </div>
      ...
      {kb.selectedEntry && (
        <div className='selected-entry'>
          ...
          <div className='entry-details'>
            <p>
              <strong>Problem:</strong> {kb.selectedEntry.problem}
            </p>
            <p>
              <strong>Solution:</strong> {kb.selectedEntry.solution}
            </p>
            <p>
              <strong>Tags:</strong> {kb.selectedEntry.tags?.join(', ') || 'None'}
            </p>
          </div>
  ...
  return (
    <div className='metrics-section'>
      ...
      <div className='metrics-status'>
        ...
        {metrics.isDataStale() && <span className='stale-indicator'>⚠️ Data may be stale</span>}
      </div>
      ...
      <div className='metrics-actions'>
        ...
        <button
          onClick={() =>
            metrics.isAutoRefreshing ? metrics.stopAutoRefresh() : metrics.startAutoRefresh()
          }
        >
  ...
  return (
    <div className='ipc-status'>
      ...
      <div className='ipc-actions'>
        ...
        <button onClick={ipc.toggleTheme}>Toggle Theme</button>
      </div>
    </div>
  );
}
