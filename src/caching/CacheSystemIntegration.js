...
class CacheSystemIntegration {
    ...
    async initialize() {
        try {
            this.cacheManager = new MultiLayerCacheManager_1.MultiLayerCacheManager(this.database, this.config.mvpLevel, {
            ...
            this.warmingEngine = new CacheWarmingEngine_1.CacheWarmingEngine(this.database, this.cacheManager, this.config.mvpLevel);
            this.invalidationManager = new CacheInvalidationManager_1.CacheInvalidationManager(this.database, this.cacheManager);
            this.performanceMonitor = new CachePerformanceMonitor_1.CachePerformanceMonitor(this.database, this.cacheManager, this.warmingEngine, this.invalidationManager, this.config.mvpLevel);
    ...
    async get(key, computeFn, options) {
        ...
        try {
            if (options?.userContext) {
                await this.warmingEngine.learnFromInteraction(options.userContext, key, options.category || 'general', 0, new Date());
            }
            ...
            if (options?.userContext) {
                await this.warmingEngine.learnFromInteraction(options.userContext, key, options.category || 'general', duration, new Date());
            }
            return result;
        }
        catch (error) {
