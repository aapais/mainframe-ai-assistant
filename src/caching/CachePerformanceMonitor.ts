...
export class CachePerformanceMonitor extends EventEmitter {
  ...
  private calculatePerformanceGrade(slaCompliance: number, metrics: CacheMetrics): 'A' | 'B' | 'C' | 'D' | 'F' {
  ...
  private async storeOperationMetric(operation: string, duration: number, cacheHit: boolean): Promise<void> {
    try {
      this.db.prepare(`
