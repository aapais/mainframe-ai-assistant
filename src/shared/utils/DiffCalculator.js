...
class DiffCalculator {
        ...
        for (let oldIndex = 0; oldIndex < oldArray.length; oldIndex++) {
            ...
            if (!usedOldIndices.has(oldIndex)) {
                ...
                if (bestMatch !== -1 && bestScore < 0.7) {
                    ...
                    await this.calculateDiffRecursive(oldArray[oldIndex], newArray[bestMatch], itemPath, diff, depth + 1);
                }
                else {
                    diff.deleted.push({
                        path: itemPath,
    ...
    optimizeDiff(diff) {
        ...
        optimized.modified = optimized.modified.filter(item => !parentPaths.has(item.path.split('.').slice(0, -1).join('.')));
        return optimized;
    }
