...
export class SecurityHeadersMiddleware {
  ...
  private getDefaultConfig(): SecurityHeadersConfig {
    return {
      csp: {
        enabled: true,
        policy: "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self' wss: ws:; frame-ancestors 'none';",
        reportOnly: false
      },
      hsts: {
        enabled: true,
  ...
  private mergeConfig(userConfig: Partial<SecurityHeadersConfig>): void {
    ...
    if (userConfig.permissionsPolicy) {
      this.config.permissionsPolicy = { ...this.config.permissionsPolicy!, ...userConfig.permissionsPolicy };
    }
...
export const securityHeaders = {
  ...
  auth: new SecurityHeadersMiddleware({
    csp: {
      enabled: true,
      policy: "default-src 'none'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self'; connect-src 'self'; frame-ancestors 'none'; form-action 'self';"
    },
    frameOptions: {
      enabled: true,
      policy: 'DENY'
  ...
  development: new SecurityHeadersMiddleware({
    csp: {
      enabled: true,
      policy: "default-src 'self' 'unsafe-inline' 'unsafe-eval' data: blob:; connect-src 'self' ws: wss:;",
      reportOnly: true
    },
    hsts: {
      enabled: false
