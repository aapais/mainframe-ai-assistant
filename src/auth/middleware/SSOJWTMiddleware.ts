...
export class SSOJWTMiddleware {
   ...
   * Verify JWT token
   */
  private async verifyToken(token: string, type: 'access' | 'refresh'): Promise<SSOJWTPayload | null> {
    try {
  ...
  private async validateUserAndSession(decoded: SSOJWTPayload): Promise<User | null> {
    try {
      ...
      const user: User = {
        ...
        suspendedUntil: userResult.suspended_until ? new Date(userResult.suspended_until) : undefined,
        ...
        passwordChangedAt: userResult.password_changed_at ? new Date(userResult.password_changed_at) : undefined,
  ...
  private async validateDevice(req: AuthenticatedRequest, decoded: SSOJWTPayload) {
    ...
    try {
      ...
      return { valid: true, deviceInfo: { id: 'unknown', name: 'Unknown', type: 'unknown', trusted: false } };
    }
  }
   ...
   * Check if token needs refresh
   */
  private async checkTokenRefresh(req: AuthenticatedRequest, res: Response, decoded: SSOJWTPayload) {
    try {
