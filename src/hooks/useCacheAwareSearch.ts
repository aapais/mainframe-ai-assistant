...
import { getCacheApiClient, CacheApiResponse, SearchCacheResponse } from '../services/api/CacheApiClient';
import { useDebounce } from './useDebounce';
import { IncrementalResult, CachePerformanceMetrics, PredictiveCacheEntry } from '../services/cache/CacheTypes';
...
export interface UseCacheAwareSearchReturn {
  ...
  updatePredictionModel: (feedback: { query: string; selected: boolean; resultIndex?: number }) => void;
...
export const useCacheAwareSearch = (
  ...
  const checkCache = useCallback(async (query: string, options?: SearchOptions): Promise<{
    results: IncrementalResult[];
    metadata: any;
    fromCache: boolean;
  } | null> => {
  ...
  const storeInCache = useCallback(async (
    query: string,
    results: IncrementalResult[],
    metadata: any,
  ...
  const updatePredictionModel = useCallback((feedback: {
    query: string;
    selected: boolean;
    resultIndex?: number;
  }) => {
    ...
    if (feedback.resultIndex !== undefined) {
      existing.context.averageClickPosition = (
        (existing.context.averageClickPosition || 0) + feedback.resultIndex
      ) / 2;
    }
    ...
    cacheManager.set(CACHE_KEYS.PREDICTION_MODEL, 
      Array.from(predictionModelRef.current.entries())
    ).catch(console.warn);
  }, [config.enablePrediction, cacheManager]);
  
  const predictQueries = useCallback((partialQuery: string): string[] => {
    ...
    for (const [query, entry] of predictionModelRef.current.entries()) {
      if (query.toLowerCase().includes(partialQuery.toLowerCase())) {
        const recencyFactor = Math.max(0, 1 - (Date.now() - entry.lastUsed) / (24 * 60 * 60 * 1000));
  ...
  const prefetchPredictions = useCallback(async (currentQuery: string): Promise<void> => {
    if (!config.enablePrediction) return;
      ...
      .map(async (query) => {
        try {
          ...
          if (!cached) {
            ...
            apiClient.search({ query, options: { limit: config.defaultLimit } })
              .then(response => {
  ...
  }, [config.enablePrediction, config.defaultLimit, predictQueries, checkCache, apiClient, storeInCache]);
  ...
  const performSearch = useCallback(async (
    ...
    try {
      ...
      if (!fromCache && !isLoadMore) {
        await storeInCache(query, data.results, {
      ...
      const averageResponseTime = responseTimesRef.current.reduce((sum, time) => sum + time, 0) / responseTimesRef.current.length;
      setMetrics(prev => ({
        ...prev,
        responseTime,
        averageResponseTime,
      ...
      if (!searchPatternsRef.current.includes(query)) {
        ...
        cacheManager.set(CACHE_KEYS.SEARCH_PATTERNS, searchPatternsRef.current)
          .catch(console.warn);
      }
  ...
  const prefetch = useCallback(async (queries: string[]): Promise<void> => {
  ...
  const warmCache = useCallback(async (queries: string[]): Promise<void> => {
    try {
  ...
  const getSuggestions = useCallback(async (partialQuery: string): Promise<any[]> => {
  ...
  useEffect(() => {
    const loadPredictionModel = async () => {
      try {
        const stored = await cacheManager.get<Array<[string, PredictiveCacheEntry]>>(CACHE_KEYS.PREDICTION_MODEL);
        if (stored) {
