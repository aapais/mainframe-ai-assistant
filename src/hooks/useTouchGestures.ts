...
const getSwipeDirection = (start: TouchPoint, end: TouchPoint, threshold: number): string | null => {
...
export const useTouchGestures = (options: TouchGestureOptions = {}): TouchGestureReturn => {
  ...
  const handleTouchMove = useCallback((event: TouchEvent) => {
    ...
    if (longPressTimer && touches.length === 1 && startPoints.length === 1) {
      ...
      if (distance > 10) { // 10px threshold for movement cancellation
    ...
    if (enablePullToRefresh && touches.length === 1 && startPoints.length === 1) {
      ...
      if (dy > 100 && window.scrollY === 0) { // 100px threshold at top of page
  ...
  const handleTouchEnd = useCallback((event: TouchEvent) => {
    const { startPoints, currentPoints, gestureType, longPressTimer, lastTapTime, tapCount } = gestureRef.current;
      ...
      if (timeDiff <= swipeTimeout) {
        ...
        if (!direction) {
          ...
          if (timeSinceLastTap < 300 && tapCount === 1) {
            ...
            setTimeout(() => {
              if (gestureRef.current.tapCount === 1 && gestureRef.current.lastTapTime === currentTime) {
                if (enableHaptics) hapticFeedback('selection');
