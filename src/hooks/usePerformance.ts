...
export const usePerformance = (options: UsePerformanceOptions = {}): UsePerformanceReturn => {
  ...
  const [componentMetrics, setComponentMetrics] = useState<Map<string, ComponentPerformance>>(
    new Map()
  );
  ...
  const activeMeasures = useRef<
    Map<string, { name: string; startTime: number; metadata?: Record<string, any> }>
  >(new Map());
  ...
  const startMeasure = useCallback(
    (name: string, metadata?: Record<string, any>): string => {
  ...
  const recordMetric = useCallback(
    (name: string, value: number, type: string = 'custom', metadata?: Record<string, any>) => {
      const metric: PerformanceMetric = {
        name,
        value,
        timestamp: Date.now(),
  ...
  const recordNetworkMetric = useCallback(
  ...
  const recordCustomMeasure = useCallback(
  ...
  const checkBudgetViolation = useCallback((metric: string, actual: number, expected: number) => {
    const severity = actual > expected * 2 ? 'high' : actual > expected * 1.5 ? 'medium' : 'low';
  ...
  const budgetViolations = useMemo(() => {
    ...
    const renderViolations = recentMetrics.filter(
      m => m.type === 'render' && m.value > budget.render_time
    ).length;
    ...
    const networkViolations = networkMetrics.filter(
      m => m.duration > budget.network_request
    ).length;
  ...
  const getMetricHistory = useCallback(
    (metricName: string, limit: number = 50): PerformanceMetric[] => {
      return metrics
  ...
  const getSlowComponents = useCallback(
    (threshold: number = budget.render_time): ComponentPerformance[] => {
  ...
  const getPerformanceScore = useCallback((): number => {
    ...
    if (avgNetworkTime > budget.network_request) {
      score -= Math.min(
        25,
        ((avgNetworkTime - budget.network_request) / budget.network_request) * 30
      );
    }
    ...
    if (avgInteractionDelay > budget.interaction_delay) {
      score -= Math.min(
        10,
        ((avgInteractionDelay - budget.interaction_delay) / budget.interaction_delay) * 10
      );
    }
  ...
  const generateReport = useCallback((): PerformanceReport => {
    ...
    return {
      ...
      summary: {
        ...
        slow_renders: Array.from(componentMetrics.values()).reduce(
          (sum, comp) => sum + comp.slow_renders,
          0
        ),
