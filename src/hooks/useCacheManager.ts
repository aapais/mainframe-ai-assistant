...
export const useCacheManager = (options: UseCacheManagerOptions = {}): UseCacheManagerReturn => {
  ...
  // Record access time
  const recordAccessTime = useCallback(
    (startTime: number) => {
      ...
      const averageAccessTime =
        accessTimesRef.current.reduce((sum, time) => sum + time, 0) / accessTimesRef.current.length;
  ...
  const setToStorage = useCallback(
    async <T>(
      entry: CacheEntry<T>,
      storage: 'localStorage' | 'sessionStorage' | 'auto' = 'auto'
    ): Promise<void> => {
      try {
      ...
      try {
        ...
        if (memoryEntry) {
          if (memoryEntry.ttl === 0 || Date.now() - memoryEntry.created < memoryEntry.ttl) {
            ...
            const hitRate =
              metricsRef.current.totalRequests > 0
  ...
  const set = useCallback(
    async <T>(key: string, value: T, options: CacheOptions = {}): Promise<void> => {
      ...
      try {
        ...
        const currentMemoryUsage = Array.from(memoryCache.current.values()).reduce(
        ...
        const memoryUsage = Array.from(memoryCache.current.values()).reduce(
          (sum, entry) => sum + entry.size,
          0
        );
  ...
  const getMany = useCallback(
    async <T>(keys: string[]): Promise<Array<T | null>> => {
  ...
  const setMany = useCallback(
    async <T>(entries: Array<{ key: string; value: T; options?: CacheOptions }>): Promise<void> => {
  ...
  const evict = useCallback(
    async (strategy: 'lru' | 'lfu' | 'ttl' | 'size' = 'lru'): Promise<number> => {
      const entries = Array.from(memoryCache.current.entries());
  ...
  const warm = useCallback(
    async (entries: Array<{ key: string; loader: () => Promise<any> }>): Promise<void> => {
      setIsLoading(true);
