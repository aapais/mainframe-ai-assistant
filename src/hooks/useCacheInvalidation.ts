...
export interface InvalidationAction {
  type: 'invalidate_keys' | 'invalidate_pattern' | 'refresh_data' | 'notify_components' | 'clear_section';
  config: {
    keys?: string[];
    pattern?: string;
    component?: string;
...
export interface UseCacheInvalidationReturn {
  ...
  // Manual invalidation
  invalidateKeys: (keys: string[], options?: { silent?: boolean; cascade?: boolean }) => Promise<void>;
  invalidatePattern: (pattern: string, options?: { silent?: boolean; cascade?: boolean }) => Promise<void>;
  invalidateByTags: (tags: string[], options?: { silent?: boolean; cascade?: boolean }) => Promise<void>;
  invalidateAll: (options?: { silent?: boolean; confirm?: boolean }) => Promise<void>;
  
  // Trigger-based invalidation
  triggerDataChange: (dataPath: string, changeType: 'create' | 'update' | 'delete', data?: any) => void;
...
export const useCacheInvalidation = (
  ...
  const addStrategy = useCallback((strategy: InvalidationStrategy) => {
  ...
  const updateStrategy = useCallback((strategyId: string, updates: Partial<InvalidationStrategy>) => {
  ...
  const invalidateKeysInternal = useCallback(async (
    keys: string[],
  ...
  const invalidatePatternInternal = useCallback(async (
    pattern: string,
    options: { silent?: boolean; cascade?: boolean } = {}
  ): Promise<string[]> => {
  ...
  const invalidateAll = useCallback(async (
    options: { silent?: boolean; confirm?: boolean } = {}
  ): Promise<void> => {
  ...
  const triggerDataChange = useCallback((dataPath: string, changeType: 'create' | 'update' | 'delete', data?: any) => {
  ...
  const processTriggeredInvalidation = useCallback((trigger: InvalidationTrigger, context?: any) => {
    for (const strategy of strategiesRef.current.values()) {
      // Check if strategy matches trigger
      const matchingTrigger = strategy.triggers.find(t => 
        t.type === trigger.type && matchesTriggerConfig(t, trigger)
      );
  ...
  const matchesTriggerConfig = useCallback((strategyTrigger: InvalidationTrigger, actualTrigger: InvalidationTrigger): boolean => {
    const sConfig = strategyTrigger.config;
    const aConfig = actualTrigger.config;
    
    switch (strategyTrigger.type) {
      case 'data_change':
        return (!sConfig.dataPath || new RegExp(sConfig.dataPath.replace('*', '.*')).test(aConfig.dataPath || '')) &&
               (!sConfig.changeType || sConfig.changeType === 'any' || sConfig.changeType === aConfig.changeType);
      ...
      case 'api_response':
        return (!sConfig.endpoint || sConfig.endpoint === aConfig.endpoint) &&
  ...
  const evaluateConditions = useCallback((conditions: InvalidationCondition[], context?: any): boolean => {
    return conditions.every(condition => {
      switch (condition.type) {
        case 'pattern_match':
          return condition.config.pattern ? 
            new RegExp(condition.config.pattern).test(context?.data || '') : true;
  ...
  const endBatch = useCallback(async (execute: boolean = true): Promise<void> => {
  ...
  const getInvalidationStats = useCallback(() => {
    ...
    const averageDuration = totalInvalidations > 0 
      ? events.reduce((sum, e) => sum + e.duration, 0) / totalInvalidations 
      : 0;
    
    const strategyCounts = events.reduce((acc, event) => {
  ...
  const setupStrategyTriggers = useCallback((strategy: InvalidationStrategy) => {
