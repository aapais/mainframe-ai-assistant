...
export const useKnowledgeBase = () => {
  ...
  const addEntry = useCallback(async (entry: Omit<SearchResult, 'id' | 'created_at' | 'updated_at'>): Promise<boolean> => {
    setLoading(true);
    setError(null);
  ...
  const updateEntry = useCallback(async (id: number, entry: Partial<SearchResult>): Promise<boolean> => {
    setLoading(true);
    setError(null);
