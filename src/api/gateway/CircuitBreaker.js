...
class CircuitBreaker extends EventEmitter {
  ...
  async recordFailure(requestId, error, startTime) {
    ...
    logger.warn(
      `Circuit breaker failure: ${this.options.name} - ${error.message} (${this.failureCount}/${this.options.failureThreshold})`
    );
  }
  ...
  performHealthCheck() {
    try {
      ...
      this.failures = this.failures.filter(f => new Date(f.timestamp).getTime() > oneHourAgo);

      this.successes = this.successes.filter(s => new Date(s.timestamp).getTime() > oneHourAgo);
  ...
  getMetrics() {
    ...
    const recentFailures = this.failures.filter(f => new Date(f.timestamp).getTime() > lastHour);

    const recentSuccesses = this.successes.filter(s => new Date(s.timestamp).getTime() > lastHour);
