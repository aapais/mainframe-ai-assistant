...
const enhancedAuthMiddleware = async (req, res, next) => {
  try {
    ...
    logger.debug(
      `Usuário autenticado: ${user.id} - ${req.method} ${req.path} (${processingTime}ms)`
    );
  ...
  return (req, res, next) => {
    try {
      ...
      const hasRequiredRole = allowedRoles.some(
        role => userRoles.includes(role) || req.user.isAdmin
      );

      if (!hasRequiredRole) {
        logger.warn(
          `Acesso negado para usuário ${req.user.id}. Roles necessárias: ${allowedRoles.join(', ')}`
        );
...
const permissionMiddleware = requiredPermissions => {
  return (req, res, next) => {
    try {
      ...
      const neededPermissions = Array.isArray(requiredPermissions)
        ? requiredPermissions
        : [requiredPermissions];
      ...
      if (!hasAllPermissions) {
        logger.warn(
          `Permissões insuficientes para usuário ${req.user.id}. Necessárias: ${neededPermissions.join(', ')}`
        );
