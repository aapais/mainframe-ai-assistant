...
const adminMiddleware = (req, res, next) => {
  try {
    ...
    if (!req.user.isAdmin) {
      logger.warn(
        `Tentativa de acesso admin negada para usuário ${req.user.id} (${req.user.email})`
      );
  ...
  return (req, res, next) => {
    try {
      ...
      if (!hasAllPermissions) {
        logger.warn(
          `Permissões múltiplas negadas para usuário ${req.user.id}: ${requiredPermissions.join(', ')}`
        );
...
const anyPermissionMiddleware = alternativePermissions => {
  return (req, res, next) => {
    try {
      ...
      const hasAnyPermission = alternativePermissions.some(
        permission => req.user.permissions && req.user.permissions.includes(permission)
      );

      if (!hasAnyPermission) {
        logger.warn(
          `Nenhuma permissão alternativa encontrada para usuário ${req.user.id}: ${alternativePermissions.join(', ')}`
        );
