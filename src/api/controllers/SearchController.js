...
class SearchController {
  ...
  async performSemanticSearch(searchParams, user) {
    try {
      ...
      const [knowledgeResults, incidentResults, solutionResults] = await Promise.all([
  ...
  async findSimilarContent(searchParams, user) {
    try {
      ...
      const { incidentId, limit = 5, includeResolved = true } = searchParams;
  ...
  async getSearchSuggestions(suggestParams, user) {
    try {
      ...
      const { partial = '', limit = 10 } = suggestParams;

      // Obter sugestões de diferentes fontes
      const [historySuggestions, trendingSuggestions, autocompleteSuggestions] = await Promise.all([
  ...
  async submitSearchFeedback(feedbackData, user) {
    try {
      ...
      const { searchId, resultId, relevance, helpful = null, comments = '' } = feedbackData;
  ...
  async getSearchAnalytics(analyticsParams, user) {
    try {
      ...
      const { timeframe = '7d', metric = 'all' } = analyticsParams;
