...
router.post(
  ...
  [
    ...
    body('isPublic').optional().isBoolean().withMessage('isPublic deve ser booleano'),
  ],
  async (req, res) => {
    try {
      const errors = validationResult(req);
...
router.get(
  '/articles',
  [
    query('page').optional().isInt({ min: 1 }).withMessage('Página deve ser positiva'),
    query('limit')
      .optional()
      .isInt({ min: 1, max: 100 })
...
router.get(
  '/articles/:id',
  [param('id').notEmpty().withMessage('ID do artigo é obrigatório')],
  async (req, res) => {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
...
router.post(
  ...
  [
    ...
    body('isHelpful').isBoolean().withMessage('isHelpful deve ser booleano'),
  ],
  async (req, res) => {
    try {
      const errors = validationResult(req);
