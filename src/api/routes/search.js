...
router.get(
  ...
  [
    ...
    query('includeContext').optional().isBoolean().withMessage('includeContext deve ser boolean'),
    query('category')
      .optional()
      .isString()
      .isLength({ max: 100 })
...
router.get(
  '/similar',
  [
    query('incidentId').notEmpty().withMessage('ID do incidente é obrigatório'),
    query('limit')
      .optional()
      .isInt({ min: 1, max: 20 })
      .withMessage('Limite deve ser entre 1 e 20'),
    query('includeResolved').optional().isBoolean().withMessage('includeResolved deve ser boolean'),
  ],
  validationMiddleware,
  async (req, res) => {
    try {
...
router.post(
  '/feedback',
  [
    query('searchId').notEmpty().withMessage('ID da busca é obrigatório'),
    query('resultId').notEmpty().withMessage('ID do resultado é obrigatório'),
    query('relevance').isFloat({ min: 0, max: 1 }).withMessage('Relevância deve ser entre 0 e 1'),
    query('helpful').optional().isBoolean().withMessage('helpful deve ser boolean'),
    query('comments')
      .optional()
      .isString()
      .isLength({ max: 500 })
