...
export class ValidationEngine {
   ...
   */
  async validateEntry(entry: Partial<KBEntry>, context?: ValidationContext): Promise<ValidationResult> {
    ...
    for (const [fieldName, fieldConfig] of this.fieldConfigs) {
      schema[fieldName] = {
        ...
        rules: fieldConfig.rules.map(rule => ({
          name: rule.name,
          errorMessage: typeof rule.errorMessage === 'string' 
            ? rule.errorMessage 
  ...
  async validateBatch(
    ...
    const results = await Promise.all(
      entries.map(entry => this.validateEntry(entry, context))
    );
    return results;
  }
    ...
    try {
      ...
      if (!result.isValid && result.errors.length > 0) {
        result.errors = result.errors.map(error => ({
          ...error,
          message: typeof rule.errorMessage === 'function' 
  ...
  private async validateCrossFields(
    ...
    if (entry.category && entry.tags) {
      ...
      const categoryTag = entry.tags.find(tag => 
  ...
  private extractKeywords(text: string): string[] {
    ...
    const importantTerms = [
      'JCL', 'VSAM', 'DB2', 'CICS', 'IMS', 'COBOL', 'TSO', 'ISPF',
      'S0C7', 'S0C4', 'SQLCODE', 'ABEND', 'STATUS', 'ERROR'
    ];
    ...
    const keywords = words.filter(word => 
      importantTerms.some(term => word.toUpperCase().includes(term)) ||
      word.length > 4
    );
        ...
        },
        {
          ...
          validate: (value) => {
            ...
            const warnings = !hasKeywords ? [{
              ...
              message: 'Consider including descriptive keywords like "error", "status", or "issue"',
      ...
      rules: [
        ...
        },
        {
          ...
          validate: (value) => {
            ...
            const warnings = !hasSteps ? [{
              ...
              suggestion: 'Use numbered steps (1., 2., 3.) or bullet points for better readability'
            }] : [];
        ...
        },
        {
          ...
          validate: (value) => {
            ...
            const warnings = !hasVerification && str.length > 100 ? [{
              field: 'solution',
        ...
        },
        {
          ...
          validate: (value) => {
            const validCategories = [
              'JCL', 'VSAM', 'DB2', 'Batch', 'Functional', 'CICS', 'IMS', 
              'TSO/ISPF', 'RACF', 'System', 'Network', 'Other'
            ];
            const str = String(value || '').trim();
  ...
  private initializeMessageTemplates(): void {
    ...
    this.messageTemplates.set('SOLUTION_RELEVANCE', 'Solution might not be relevant to the problem. Consider including keywords: {keywords}');
    this.messageTemplates.set('MISSING_CATEGORY_TAG', 'Consider adding "{category}" as a tag to match the category');
  }
