...
export async function schemaValidationExample() {
  ...
  const validEntry: Partial<KBEntry> = {
    ...
    problem:
      'Job abends with VSAM status code 35 when trying to open the customer master file. The error occurs during the nightly batch processing and affects multiple programs that read from CUST.MASTER.VSAM dataset.',
    solution:
  ...
  if (!createResult.isValid) {
    console.log(
      'Errors:',
      createResult.errors.map(e => `${e.field}: ${e.message}`)
    );
  }
  ...
  const updateEntry = {
    ...validEntry,
    id: 'kb-123',
    title: 'Updated Title - VSAM Status 35 Error',
  };
...
export async function customValidationExample() {
          ...
          ? [
              {
                ...
                message:
                  'Must contain a valid mainframe error code (S0C7, IEF212I, SQLCODE -904, etc.)',
                value,
              },
            ]
          : [],
  ...
  validator.registerField({
    name: 'errorCode',
    rules: [StringValidators.required('Error code is required'), errorCodeValidator],
  });
...
export async function asyncValidationExample() {
  ...
  validator.registerField({
    name: 'title',
    rules: [...KBValidationRules.title, uniqueTitleValidator],
  });
...
export async function i18nValidationExample() {
  ...
  console.log(
    'English errors:',
    enResult.errors.map(e => e.message)
  );
  console.log(
    'Portuguese errors:',
    ptResult.errors.map(e => e.message)
  );
  console.log(
    'Spanish errors:',
    esResult.errors.map(e => e.message)
  );
...
export async function performanceMonitoringExample() {
  ...
  const testEntries = [
    {
      title: 'Valid Title',
      problem: 'Valid problem description with sufficient detail',
      solution: '1. Step one\n2. Step two',
      category: 'JCL',
    },
    {
      title: 'Another Valid Entry',
      problem: 'Another problem with good description and context',
      solution: '1. Check this\n2. Verify that',
      category: 'VSAM',
    },
...
export async function completeFormValidationExample() {
  ...
  const formData = {
    ...
    problem:
      'Transaction abends with ASRA (0C4) when processing customer inquiry. Error occurs in CUSTINQ program at offset +000234. The abend happens intermittently, usually during high-volume periods.',
    solution:
      '1. Review the compile listing at offset +000234\n2. Check for uninitialized pointer or addressing issue\n3. Enable CEDF trace for detailed analysis\n4. Verify WORKING-STORAGE initialization\n5. Check COMMAREA length and usage\n6. Test with CICS trace if needed',
    category: 'CICS',
    severity: 'high',
