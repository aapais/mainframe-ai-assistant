...
export class ValidationI18n {
   ...
   */
  getMessage(key: string, params?: Record<string, any>, locale?: string): string {
    ...
    const portuguesePack: LanguagePack = {
      ...
      messages: {
        ...
        SOLUTION_RELEVANCE: {
          key: 'SOLUTION_RELEVANCE',
          message:
            'A solução pode não ser relevante para o problema. Considere incluir palavras-chave: {keywords}',
    ...
    const spanishPack: LanguagePack = {
      ...
      messages: {
        ...
        TITLE_DESCRIPTIVE: {
          key: 'TITLE_DESCRIPTIVE',
          message:
            'El título debería incluir palabras descriptivas como "error", "estado" o "problema"',
          description: 'Sugerencia de calidad del título',
        },
        SOLUTION_RELEVANCE: {
          key: 'SOLUTION_RELEVANCE',
          message:
            'La solución podría no ser relevante para el problema. Considere incluir palabras clave: {keywords}',
 ...
 */
export function withI18n<T>(messageKey: string, defaultMessage: string): (rule: any) => any {
