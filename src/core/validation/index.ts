...
// Schema-based validation
export { SchemaValidator, ValidationSchema, PropertySchema } from './SchemaValidator';
...
export function createValidationSuite(locale: string = 'en') {
  ...
  return {
    ...
    validateField: (field: string, value: any, context?: any) =>
      engine.validateField(field, value, context),
    validateWithSchema: (schema: string, data: any, context?: any) =>
      schemaValidator.validateAgainstSchema(schema, data, context),
...
export const KBValidationRules = {
  ...
  problem: [
    ...
    StringValidators.mainframeTerms(
      'Include mainframe terminology and error codes when applicable'
    ),
  ],
...
export class ValidationErrorFormatter {
  ...
  static toMarkdown(result: ValidationResult): string {
    ...
    if (result.errors.length > 0) {
      ...
      sections.push(...result.errors.map(error => `- **${error.field}**: ${error.message}`));
      sections.push('');
    }
...
export class ValidationPerformanceMonitor {
  ...
  recordValidation(type: string, duration: number, success: boolean, errorCount: number = 0): void {
