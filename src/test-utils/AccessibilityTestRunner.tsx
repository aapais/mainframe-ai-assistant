...
export class AccessibilityTestRunner {
  ...
  async runAllTests(): Promise<AccessibilityTestReport> {
    ...
    for (const suite of this.testSuites) {
      ...
      report.summary.passed += suiteResult.tests.filter(t => t.result.passed && t.result.severity !== 'warning').length;
      report.summary.failed += suiteResult.tests.filter(t => !t.result.passed).length;
      report.summary.warnings += suiteResult.tests.filter(t => t.result.severity === 'warning').length;
    }
  ...
  private async testColorContrast(): Promise<TestResult> {
    ...
    const failedElements = contrastResults.filter(r => r.impact === 'serious' || r.impact === 'critical');
  ...
  private async testHighContrastMode(): Promise<TestResult> {
    ...
    const hasHighContrastStyles = stylesheets.some(sheet => 
  ...
  private async testLinkAccessibility(): Promise<TestResult> {
    ...
    return {
      ...
      message: problematicLinks.length === 0 
