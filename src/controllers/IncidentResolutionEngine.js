...
class IncidentResolutionEngine extends EventEmitter {
  ...
  async processIncident(incidentData) {
    ...
    try {
      ...
      this.logger.info(
        `Incidente ${incidentId} processado com sucesso em ${result.processingTime}ms`
      );
        ...
        try {
          if (service.healthCheck) {
            ...
            healthStatus.services[name] = {
              status: 'unknown',
              message: 'Health check não implementado',
            };
          }
        } catch (error) {
          healthStatus.services[name] = {
  ...
  updateMetrics(result) {
    ...
    if (result.status === 'processed') {
      ...
      const newAvg =
        (currentAvg * (this.metrics.resolvedIncidents - 1) + result.processingTime) /
        this.metrics.resolvedIncidents;
