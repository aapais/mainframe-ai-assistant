...
class AlertingEngine extends events_1.EventEmitter {
    ...
    getStatistics() {
        ...
        const alertsLast24h = this.db.prepare(`
      SELECT COUNT(*) as count FROM alerts 
      WHERE triggered_at > datetime('now', '-24 hours')
    `).get();
        const topMetrics = this.db.prepare(`
    ...
    sendConsoleNotification(alert) {
        ...
        console.log(`   Metric: ${alert.metric} = ${alert.current_value} (threshold: ${alert.threshold})`);
    ...
    async checkEscalation(alert, rule) {
        ...
        for (const stage of rule.escalation.stages) {
            if (timeSinceTriggered >= stage.delay) {
                console.log(`📈 Escalating alert ${alert.id} to stage with channels: ${stage.channels.join(', ')}`);
    ...
    storeRule(rule) {
      ...
      `).run(rule.id, rule.name, rule.description, rule.enabled ? 1 : 0, rule.metric, rule.operator, rule.threshold, rule.duration, rule.severity, JSON.stringify(rule.channels), rule.escalation ? JSON.stringify(rule.escalation) : null, rule.cooldown, rule.schedule ? JSON.stringify(rule.schedule) : null, JSON.stringify(rule.tags), rule.runbook, rule.created_at.toISOString(), rule.updated_at.toISOString());
        }
        catch (error) {
    ...
    storeChannel(channel) {
      ...
      `).run(channel.id, channel.name, channel.type, JSON.stringify(channel.config), channel.enabled ? 1 : 0, channel.rate_limit ? JSON.stringify(channel.rate_limit) : null);
        }
        catch (error) {
    ...
    storeAlert(alert) {
      ...
      `).run(alert.id, alert.rule_id, alert.message, alert.severity, alert.metric, alert.current_value, alert.threshold, alert.status, alert.acknowledged_by, alert.acknowledged_at?.toISOString() || null, alert.resolved_at?.toISOString() || null, alert.triggered_at.toISOString(), alert.last_notification.toISOString(), alert.notification_count, alert.query, JSON.stringify(alert.tags), JSON.stringify(alert.context));
        }
        catch (error) {
