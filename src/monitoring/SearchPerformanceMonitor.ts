...
export class SearchPerformanceMonitor extends EventEmitter {
  ...
  getDashboardData(): {
    ...
    const recentTrends = this.getPerformanceTrends(24); // Last 24 hours
  ...
  getPerformanceTrends(hours: number = 24): {
    responseTime: Array<{ time: Date; avg: number; p95: number; p99: number }>;
    throughput: Array<{ time: Date; qps: number }>;
   ...
   */
  getTopQueries(limit: number = 10): QueryProfileData[] {
   ...
   * Get slowest queries
   */
  getSlowQueries(limit: number = 10): QueryProfileData[] {
    return Array.from(this.queryProfiles.values())
  ...
  /**
   * Get queries violating SLA
   */
  getSLAViolations(limit: number = 20): Array<{
    query: string;
    avgTime: number;
    executions: number;
  ...
  private updateQueryProfile(data: {
    query: string;
    normalizedQuery: string;
    duration: number;
    cacheHit: boolean;
  ...
  private checkSearchAlert(query: string, duration: number, timestamp: Date): void {
  ...
  private async checkMetricAlerts(metrics: SearchMetrics): Promise<void> {
  ...
  private createAlert(alert: Omit<SearchAlert, 'id' | 'timestamp'>): void {
  ...
  private calculateCacheHitRateByType(): Record<string, number> {
    // Would need actual cache hit tracking by strategy type
    return {
      exact: 0.95,
      fts: 0.75,
      fuzzy: 0.6,
  ...
  private getTopQueriesForMetrics(): Array<{ query: string; count: number; avgTime: number }> {
  ...
  private getSlowQueriesForMetrics(): Array<{ query: string; time: number; timestamp: Date }> {
  ...
  private storeSearchMetric(data: {
    query: string;
    duration: number;
    resultCount: number;
    cacheHit: boolean;
