...
class PerformanceProfiler {
    ...
    profileQuery(sessionId, query, strategy, executionFn) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                perf_hooks_1.performance.measure(`query-duration-${queryId}`, `query-start-${queryId}`, `query-end-${queryId}`);
    ...
    calculatePerformanceScore(session) {
        ...
        return Math.round(responseTimeScore * weights.response +
    ...
    storeSession(session) {
      ...
      `).run(session.id, session.name, session.startTime, session.endTime || null, session.duration || null, JSON.stringify(session.queries), JSON.stringify(session.memorySnapshots), JSON.stringify(session.cpuProfile), JSON.stringify(session.bottlenecks), JSON.stringify(session.recommendations), JSON.stringify(session.metrics));
        }
        catch (error) {
