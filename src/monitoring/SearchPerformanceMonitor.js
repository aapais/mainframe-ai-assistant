...
class SearchPerformanceMonitor extends events_1.EventEmitter {
    ...
    getPerformanceTrends(hours = 24) {
        ...
        for (let time = startTime; time < Date.now(); time += bucketSize) {
            const bucketData = historicalData.filter(m => m.timestamp >= time && m.timestamp < time + bucketSize);
    ...
    async collectMetrics() {
        try {
            const timestamp = new Date();
            const recentData = this.responseTimes.filter(rt => rt.timestamp.getTime() > Date.now() - this.config.monitoringInterval);
    ...
    calculateSearchAccuracy() {
        const recentQueries = Array.from(this.queryProfiles.values())
            .filter(q => q.lastExecuted.getTime() > Date.now() - 3600000);
    ...
    async storeMetrics(metrics) {
      ...
      `).run(metrics.timestamp.toISOString(), metrics.avgResponseTime, metrics.p50ResponseTime, metrics.p95ResponseTime, metrics.p99ResponseTime, metrics.queriesPerSecond, metrics.searchAccuracy, metrics.cacheHitRate, metrics.indexSize, metrics.indexGrowthRate, metrics.slaCompliance, metrics.slaViolations);
        }
        catch (error) {
    ...
    storeSearchMetric(data) {
      ...
      `).run(data.timestamp.toISOString(), data.query, data.duration, data.resultCount, data.cacheHit ? 1 : 0, data.strategy, data.indexesUsed);
        }
        catch (error) {
    ...
    storeAlert(alert) {
      ...
      `).run(alert.id, alert.timestamp.toISOString(), alert.level, alert.metric, alert.currentValue, alert.threshold, alert.message, alert.query || null, JSON.stringify(alert.recommendations));
        }
        catch (error) {
