...
program
  .name('search-monitor')
  .description('Search Service Monitoring CLI')
  .version('1.0.0');
  ...
  .action(async () => {
    ...
    try {
      ...
      for (const [component, status] of Object.entries(health.components)) {
        const icon = status === 'connected' || status === 'active' || status === 'enabled' || 
                    status === 'configured' || status === 'available' || status === 'compliant' ? '✅' : '⚠️';
...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      console.log(`  Success rate: ${((results.totalQueries - results.slaViolations) / results.totalQueries * 100).toFixed(1)}%`);
